...
* `MIN` and `MAX` skip `NaN` samples. Over a window that holds at least one real value they return that value; over
  a window whose samples are *all* `NaN` they return `NaN`, meaning the window has no measurement to report.
* `SUM` and `AVG` currently return `NaN` if any sample in the window is `NaN`.
* A genuine `Infinity` or `-Infinity` sample is a measurement and is returned as such.

Over the REST and Grafana endpoints an absent `MIN`/`MAX` is serialized as JSON `null`, since JSON has no `NaN`
literal. A Grafana panel therefore draws a gap for such a bucket rather than a dip to zero.

[NOTE]
====
Before 26.9.1 an all-`NaN` window returned an internal placeholder instead - `1.7976931348623157E308` from the
multi-column aggregation, `+Inf`/`-Inf` from PromQL's `min_over_time`/`max_over_time` - and those placeholders were
indistinguishable from real measurements. Different query paths could also disagree with each other on the same
data.
====
