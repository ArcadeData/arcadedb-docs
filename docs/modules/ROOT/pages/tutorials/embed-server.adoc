[[embedded-server]]
= Embedded Server
image:edit.png[link="https://github.com/ArcadeData/arcadedb-docs/blob/main/src/main/asciidoc/tutorials/embed-server.adoc" float="right"]
...
image::arcadedb-box.png[alt="ArcadeDB Box",align="center"]
...
You can easily add custom HTTP commands on ArcadeDB's Undertow HTTP Server by creating a xref:how-to/operations/server.adoc#server-plugin[Server Plugin] (look at `MongoDBProtocolPlugin` plugin implementation for a real example) and implementing the `registerAPI` method.
...
image::arcadedb-boxes.png[alt="ArcadeDB Box",align="center"]
