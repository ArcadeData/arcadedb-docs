...
image:edit.png[link="https://github.com/ArcadeData/arcadedb-docs/blob/main/src/main/asciidoc/how-to/connectivity/bolt.adoc" float=right]
...
This allows you to use the official Neo4j drivers with ArcadeDB, leveraging the native xref:reference/cypher/chapter.adoc#cypher[OpenCypher] query engine for graph operations.
...
If you're using ArcadeDB as xref:tutorials/embed-server.adoc#embedded-server[embedded], add the dependency to the `arcadedb-bolt` library.
...
To start the BOLT plugin, enlist it in the xref:reference/settings.adoc#settings-sql[`server.plugins`] settings.
...
Since BOLT protocol uses Cypher as its query language, you can execute any query supported by ArcadeDB's xref:reference/cypher/chapter.adoc#cypher[OpenCypher] implementation:
...
* xref:reference/cypher/chapter.adoc#cypher[OpenCypher Query Language]
* xref:how-to/connectivity/postgres.adoc#postgres-driver[PostgreSQL Protocol Plugin]
