[[indexes]]
= Indexes
image:edit.png[link="https://github.com/ArcadeData/arcadedb-docs/blob/main/src/main/asciidoc/concepts/indexes.adoc" float=right]

ArcadeDB supports multiple index algorithms, each optimized for different access patterns:

- **xref:reference/lsm-tree.adoc#lsm-tree[LSM Tree]** (default) — Optimized for range scans, ordered iteration, and write-heavy workloads.
...
If you're interested to ArcadeDB's LSM-Tree index implementation detail, look at xref:reference/lsm-tree.adoc#lsm-tree[LSM-Tree]
...
3. String properties can be index tokenized in a xref:how-to/data-modeling/full-text-index.adoc#full-text-index[full-text index].
...
7. WKT geometry strings can be indexed using a xref:how-to/data-modeling/geospatial.adoc#geospatial[geospatial index].
