...
The <<sql-insert-command,`INSERT`>> command creates a new record in the database. Records can be schema-less or follow rules specified in your model.
...
* *`ON DUPLICATE KEY SKIP`* When inserting multiple records, skips any record that would violate a *unique* index instead of aborting the whole command. Each result row tells you whether that record was inserted or skipped. Any other kind of error (for example, a missing mandatory field) still aborts the command as usual.
...
[WARNING]
====
`INSERT INTO <timeseries type>` is the one `INSERT` target that is **not atomic with the transaction containing
it**. The samples are committed as they are appended: they are durable and visible to every other connection as
soon as the statement returns, and rolling the transaction back does not take them back. Every other `INSERT`
target behaves normally, which is exactly why this one surprises people. See
<<timeseries-appends-and-transactions,Appends and Transactions>> for why, and for the same statement about the other ingestion paths.
====
...
Add `ON DUPLICATE KEY SKIP` if you only want to skip records that clash with a unique index,
...
* Insert several records, skipping any that clash with a unique index:

[source,sql]
----
ArcadeDB> INSERT INTO Product CONTENT [{"sku": "A1", "name": "Widget"},
            {"sku": "A2", "name": "Gadget"}] ON DUPLICATE KEY SKIP
----

If `Product.sku` is a unique index and a record with `sku = 'A1'` already exists, that record is skipped and the rest are still inserted.
...
* Inserts using <<sql-insert-command,`INSERT`>> sub-queries:
