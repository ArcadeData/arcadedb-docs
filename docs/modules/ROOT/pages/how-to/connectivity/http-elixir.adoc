...
The community has not yet released a Neo4j BOLT driver for Elixir, and the PostgreSQL Elixir libraries (`postgrex` / Ecto adapter) introspect Postgres-specific catalogs that ArcadeDB does not implement. The recommended way to talk to ArcadeDB from Elixir is therefore the xref:reference/http-api/http.adoc[*HTTP/JSON API*] using a library like https://hex.pm/packages/httpoison[HTTPoison] or `Req`.

If your team has tried BOLT or PostgreSQL from Elixir successfully, we'd love to add an example -- please https://github.com/ArcadeData/arcadedb/discussions[open a discussion].
====
