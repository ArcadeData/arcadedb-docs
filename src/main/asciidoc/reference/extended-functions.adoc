...
See <<algo-all-simple-paths>> in the Graph Algorithms appendix for full parameter reference and examples.
...
See <<algo-astar>> in the Graph Algorithms appendix for full parameter reference and examples.
...
See <<algo-dijkstra>> in the Graph Algorithms appendix for full parameter reference and examples.
...
*Syntax:* `vector.neighbors(<index-spec>, <query-vector>, <k> [, <efSearch>])`
...
* `efSearch` - (optional) Search beam width. Controls the trade-off between recall and speed. Higher values improve recall but increase latency. When omitted, the index uses <<adaptive-efsearch,adaptive efSearch>>.
...
-- Explicit efSearch for higher recall
