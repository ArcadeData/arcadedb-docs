...
WHERE vectorNeighbors('embedding', [0.1, 0.2, 0.3, ...], 10) > 0.8;
----

The `vectorNeighbors()` function returns similarity scores, allowing you to filter results by minimum similarity threshold.
...
WHERE source = 'wikipedia'
  AND vectorNeighbors('embedding', [0.1, 0.2, 0.3, ...], 10) > 0.8
ORDER BY vectorNeighbors('embedding', [0.1, 0.2, 0.3, ...], 10) DESC
LIMIT 5;
----
...
SELECT vectorNeighbors('Document[embedding]', [0.1, 0.2, 0.3, ...], 10) AS neighbors;
----
...
if (!database.getSchema().existsIndex(indexName)) {
  ...
  hnswIndex.addAll((Collection<Item<Object, float[]>>) embeddings,
