...
|collection|`+IN+`|contains any of the elements listed. Uses the index on the property, including the leading columns of a composite index (`+transactionId IN (...)+` on an index over `+(transactionId, date)+`), with every list form: `+IN (a, b)+`, `+IN [a, b]+` and a bound parameter. The listed values are looked up in index order, so an `+ORDER BY+` on the indexed columns needs no extra sort step|`+name IN ['European','Asiatic']+`
|collection|`+NOT IN+`|contains all of the elements not listed. Does not use index: an index can look up the listed values, not their complement, so the type is scanned. If the type is large, add a second indexed condition to the query to drive the lookup|`+name NOT IN ['European','Asiatic']+`
...
NOTE: `+CONTAINS+` asks whether the collection on the left holds a *single* value: either one matching the condition,
or, when the right side is a plain value, one equal to it. Put a collection on the right and it is searched for as one
element, so `+tags CONTAINS ['red','blue']+` matches a row whose `+tags+` holds the nested list `+['red','blue']+`, and
not a row whose `+tags+` is `+['red','green','blue']+`. To compare two collections, use `+CONTAINSALL+` (every element
of the right is present on the left) or `+CONTAINSANY+` (at least one is). This applies equally when the right side is
produced by an expression such as `+'red blue'.split(' ')+`.
