...
The two core classes of the embedded Java API are:

* `<<databasefactory,DatabaseFactory>>` -- entry point that creates and opens databases.
* `<<database-interface,Database>>` -- main handle for transactions, queries, schema, and CRUD operations.

[[databasefactory]]
==== `DatabaseFactory` Class

It's the entry point class that allows to create and open a database.
A `DatabaseFactory` object doesn't keep any state and its only goal is creating a `<<database-interface,Database>>` instance.

WARNING: Never open the same database directory from two processes at once (for example, an embedded tool and a running ArcadeDB Server). Doing so can corrupt the database. If a server already has a database open, connect to it remotely instead of opening it with `DatabaseFactory`.
...
TIP: To tell a property that is missing from one stored as `null`, use `Result.getPropertyIfPresent(name, absentValue)` (or `Document.getIfPresent(name, absentValue)` on a record). It returns the value, `null` included, when the property exists, and `absentValue` otherwise. It does in one lookup what `hasProperty()` followed by `getProperty()` does in two _(since v26.10.1)_.
...
Returns a `Select` builder to construct and execute queries using the <<java-select-api,Native Select API>>.
This provides a zero-parsing, JIT-friendly fluent query builder as an alternative to SQL or Cypher.
...
See the <<java-select-api,Native Select API>> documentation for the full reference including operators, graph traversal, and vector search.
...
NOTE: The retry is skipped if the block already committed part of its work before failing. This happens
when the block runs a statement with an explicit batch boundary, such as `UPDATE ... BATCH n`,
`DELETE ... BATCH n` or `MOVE VERTEX ... BATCH n`: those commit every `n` records by design, so the
rollback can no longer undo the records already committed and repeating the block would apply them a
second time. In that case the exception is propagated to the caller, who is the only one who knows how
to compensate for the part that already went through. `TRUNCATE TYPE` and `REBUILD TYPE` are not
affected, because they skip their internal batching when they run inside a transaction opened by the
caller.
