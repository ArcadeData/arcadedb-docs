[[server]]
= Server
image:edit.png[link="https://github.com/ArcadeData/arcadedb-docs/blob/main/src/main/asciidoc/how-to/operations/server.adoc" float="right"]
...
To know more about this topic, look at xref:how-to/operations/chapter.adoc#security[Security].
...
Check the xref:how-to/operations/policy.adoc#security-policy[Security Policy].
...
To specify a different name define it with the setting xref:reference/settings.adoc#settings-sql[`server.name`], example:
...
| `production` | No | Brief | Auto-set to `1` if not configured | Auto-disabled if not configured
|===

The mode is controlled by the xref:reference/settings.adoc#setting-table[`server.mode`] setting with a default mode `development`.
...
* **WAL flush** (`arcadedb.txWalFlush`): set to `1` (flush without metadata) for transaction durability. If you explicitly set it to `0`, a warning is logged. See xref:concepts/transactions.adoc#wal-flush-durability[] for details.
...
Instead of starting a server and then connect to it, to create the default databases, ArcadeDB Server takes an initial default databases list by using the setting xref:reference/settings.adoc#settings-sql[`server.defaultDatabases`].
...
- xref:reference/http-api/http.adoc#http-json-api[HTTP/JSON]
- xref:how-to/connectivity/postgres.adoc#postgres-driver[Postgres]
- xref:reference/mongodb-ql/mongo.adoc#mongodb-query-language[MongoDB]
- xref:reference/redis-ql/redis.adoc#redis-query-language[Redis]
...
If you're running ArcadeDB in xref:tutorials/embed-server.adoc#embedded-server[embedded] mode, make sure you're using the logging setting by specifying the `arcadedb-log.properties` file at JVM startup:
