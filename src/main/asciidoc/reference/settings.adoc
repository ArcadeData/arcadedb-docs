...
|`ha.quorum`|Default quorum: 'none', 'majority', 'all'.|String|majority
|`ha.quorumTimeout`|Timeout waiting for the quorum in milliseconds|Long|10000
|`ha.readConsistency`|Default read consistency for follower reads: 'eventual', 'read_your_writes', 'linearizable'|String|read_your_writes
|`ha.replicationLagWarning`|Log warning if replication lag exceeds this many entries|Long|1000
|`ha.raftSnapshotThreshold`|Number of entries after which a snapshot is automatically taken|Long|10000
|`ha.raftGroupCommitBatchSize`|Maximum entries to batch in a single group commit flush|Integer|500
|`ha.logVerbose`|HA logging level: 0=off, 1=basic, 2=detailed, 3=trace|Integer|0
