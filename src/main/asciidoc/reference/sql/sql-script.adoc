...
This is the canonical use case for SQL Script: a script that creates a vertex, looks up another, links them with an edge, retries on conflict, and returns the new edge. Many clients can run this in parallel against the same server -- each conflict triggers a server-side retry, no client-side bookkeeping required.
...
Note how `$account` and `$city` carry results from one statement to the next, and how the entire block between `BEGIN` and `COMMIT` is the unit that gets retried.
