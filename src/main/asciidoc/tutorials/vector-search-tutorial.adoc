...
NOTE: In production, embeddings are typically 384-1536 dimensions. This tutorial uses 4 dimensions for simplicity. For production workloads, add `quantization: 'INT8'` for significantly better search performance -- see <<quantization-performance>> below.
...
SELECT name, category FROM (
  SELECT vectorNeighbors('Product[embedding]', [0.88, 0.12, 0.78, 0.22], 3, 200) AS neighbors
)
----

See <<adaptive-efsearch,Adaptive efSearch>> for details on how the default strategy works.
