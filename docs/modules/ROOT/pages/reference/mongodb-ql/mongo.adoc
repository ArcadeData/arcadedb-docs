...
image:edit.png[link="https://github.com/ArcadeData/arcadedb-docs/blob/main/src/main/asciidoc/reference/mongodb-ql/mongo.adoc" float=right]

ArcadeDB provides support for both MongoDB Query Language and MongoDB protocol.

If you're using ArcadeDB as xref:tutorials/embed-server.adoc#embedded-server[embedded], please add the dependency to the `arcadedb-mongodbw` library.
...
If you want to use MongoDB Query Language from xref:reference/java-api/java-api-local.adoc#java-api-local[Java API], you can simply keep the relevant jars in your classpath and execute a query or a command with "mongo" as language.
...
You can execute a Mongo query against ArcadeDB server by using the xref:how-to/connectivity/postgres.adoc#postgres-driver[Postgres driver] and prefixing the query with `{mongo}`.
Example:
...
You can execute a Mongo query against ArcadeDB server by using xref:reference/http-api/http.adoc#http-json-api[HTTP/JSON API].
...
To start the MongoDB plugin, enlist it in the xref:reference/settings.adoc#settings-sql[`server.plugins`] settings.
