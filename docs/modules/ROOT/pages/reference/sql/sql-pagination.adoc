...
ArcadeDB supports pagination natively. Pagination doesn't consume server side resources because no cursors are used. Only xref:concepts/basics.adoc#rid[Record ID's] are used as pointers to the physical position in the bucket.
...
Where:
- *lower-rid* is the exclusive lower bound of the range as xref:concepts/basics.adoc#rid[RID]
- *max-records* is the maximum number of records returned by the query
