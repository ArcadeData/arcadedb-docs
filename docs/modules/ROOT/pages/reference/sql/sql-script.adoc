[[sql-script]]
= SQL Script
image:edit.png[link="https://github.com/ArcadeData/arcadedb-docs/blob/main/src/main/asciidoc/reference/sql/sql-script.adoc" float=right]
...
SQL Script supports all the ArcadeDB xref:reference/sql/chapter.adoc#sql-commands[SQL Commands], plus the following:
...
NOTE: A `SET GLOBAL`-declared variable exists as long as the associated database is not closed or the server shut down.
...
* `IF(&lt;condition&gt;){ &lt;statement&gt;; [&lt;statement&gt;;]* }`. Look at <<conditional-execution,conditional execution>>.
* `WHILE(&lt;condition&gt;){ &lt;statement&gt;; [&lt;statement&gt;;]* }`. Look at <<loops,loops>>.
* `FOREACH(&lt;variable&gt; IN &lt;expression&gt;){ &lt;statement&gt;; [&lt;statement&gt;;]* }`. Look at <<loops,loops>>.
...
Example to create a new vertex in a xref:concepts/transactions.adoc#transactions[Transactions] and attach it to an existent vertex by creating a new edge between them. If a concurrent modification occurs, repeat the transaction up to 100 times:
