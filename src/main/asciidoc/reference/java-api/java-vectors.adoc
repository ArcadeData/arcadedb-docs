...
The `vectorNeighbors()` function takes the *index name* (format: `'TypeName[propertyName]'`) as its first argument and returns an array of `{record, distance}` objects. Use `expand()` to flatten the array into individual result rows. The `distance` semantics depend on the similarity metric (for COSINE: 0 = identical, higher = more different). Use named parameters (e.g., `$queryVector`) instead of hardcoded vectors for real-world applications.
...
NOTE: Vector search returns `{record, distance}` objects. To filter by additional properties (e.g., `source`), apply filters in your application code after retrieving the nearest neighbors.
...
*Note*: The first argument to `vectorNeighbors()` must always be the *index name* in `'TypeName[propertyName]'` format (e.g., `'Document[embedding]'`). The function returns an array of `{record, distance}` objects — use `expand()` to flatten the results into individual rows.
