...
ArcadeDB supports two types of persistent vector indexes for similarity search on vector embeddings:

===== HNSW Vector Index

HNSW (Hierarchical Navigable Small World) is a graph-based approximate nearest neighbor search algorithm optimized for fast similarity search across large vector datasets.
