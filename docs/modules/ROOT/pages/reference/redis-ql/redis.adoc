...
= Redis
image:edit.png[link="https://github.com/ArcadeData/arcadedb-docs/blob/main/src/main/asciidoc/reference/redis-ql/redis.adoc" float=right]
...
If you're using ArcadeDB as xref:tutorials/embed-server.adoc#embedded-server[embedded], please add the dependency to the `arcadedb-redisw` library.
...
To start the Redis plugin, enlist it in the xref:reference/settings.adoc#settings-sql[`server.plugins`] settings.
...
image::redis-api.png[align="center"]
...
The bucket name is mapped as the database name first, then type, the index or the record's xref:concepts/basics.adoc#rid[`RID`] based on the use case.
...
To retrieve the document inserted above by it xref:concepts/basics.adoc#rid[`RID`] (O(1) complexity), you can use the https://redis.io/commands/hget[HGET] Redis command:
...
This allows you to execute Redis commands through the standard ArcadeDB HTTP xref:reference/http-api/http.adoc#http-command[`/command`] endpoint by specifying `language=redis`.
