...
[NOTE]
====
`TRAVERSE ... FROM <EdgeType>` and `TRUNCATE TYPE <EdgeType>` are not yet supported on a lightweight edge type and report an error rather than silently doing nothing. Use `SELECT` and `DELETE FROM` instead.
====
