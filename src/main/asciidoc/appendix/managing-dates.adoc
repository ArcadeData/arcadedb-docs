...
In the event that these default formats are not sufficient for the needs of your application, you can customize them through <<sql-L<<sql-Alter-Database,E,`ALTER DATABASE ... DATEFORMAT`>> and <<sql-L<<sql-Alter-Database,E,`DATETIMEFORMAT`>> commands.
For instance,
...
Here, you again create a vertex for the traditional date of the foundation of Rome.  However, instead of altering the database, you format the date field in <<sql-L<<sql-Create-Vertex,E,`CREATE VERTEX`>> command.
