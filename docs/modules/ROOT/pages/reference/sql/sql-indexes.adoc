...
For more information on these, see xref:concepts/indexes.adoc#indexes[Indexes].
...
See xref:concepts/indexes.adoc#case-insensitive-indexes[] for details.
...
See xref:how-to/data-modeling/full-text-index.adoc#full-text-index[] for full documentation.
...
See xref:how-to/data-modeling/geospatial.adoc#geospatial[] for full documentation.
...
NOTE: Hash indexes use xref:concepts/indexes.adoc#hash-index[extendable hashing] and provide O(1) equality lookups — significantly faster than LSM Tree for point queries (`WHERE key = value`). However, they do not support range scans (`<`, `>`, `BETWEEN`) or ordered iteration (`ORDER BY`). Use `UNIQUE`/`NOTUNIQUE` (LSM Tree) when you need range queries or ordering.
...
See xref:how-to/data-modeling/full-text-index.adoc#full-text-index[] for analyzer configuration, Lucene query syntax, relevance scoring, and More Like This.
...
See xref:how-to/data-modeling/geospatial.adoc#geospatial[] for full documentation on geometry formats, predicate functions, and precision configuration.
