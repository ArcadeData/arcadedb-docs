...
An index counts as already existing when it covers the same properties with the same index type, either on the type named in the statement or on one of its *super types*: an index declared on a super type indexes the records of all its sub types, so there is nothing left to create.
An index of the same name on an unrelated type, on a *sub* type, on different properties, or of a different index type is still reported as an error, because none of those provides what the statement asked for.
+
[source,sql]
----
CREATE INDEX `Element_uuid` IF NOT EXISTS ON `Element` (uuid) UNIQUE;
-- ignored: the index on the super type already covers UnidentifiedElement
CREATE INDEX `Element_uuid` IF NOT EXISTS ON `UnidentifiedElement` (uuid) UNIQUE;
----
...
** `COLLATE CI` enables case-insensitive collation for this property.
Values are stored in lowercase internally, so lookups match regardless of case.
Can be specified independently for each property in a composite index.
The only accepted values are `CI` and `DEFAULT`, in any letter case; anything else is reported as an error rather than ignored, so a misspelled collation cannot silently produce a case-sensitive index.
See <<case-insensitive-indexes>> for details.
...
Uses https://lucene.apache.org[Apache Lucene] for tokenization and supports configurable analyzers, multi-property indexing, advanced query syntax, and BM25 relevance scoring.
...
** `LSM_SPARSE_VECTOR` posting-list inverted index for sparse-vector retrieval (BM25-style, SPLADE, BGE-M3, OpenSearch sparse).
See <<sparse-vector-search,Sparse Vector Search>> for full documentation.
...
* Create a series of automatic indexes for the `thumbs` map property in the type `Movie`.
`BY KEY` indexes the map's keys, `BY VALUE` its values.
They are usually `NOTUNIQUE`, since the same key (or value) normally appears across many records:

----
ArcadeDB> CREATE INDEX ON Movie (thumbs BY KEY) NOTUNIQUE
ArcadeDB> CREATE INDEX ON Movie (thumbs BY VALUE) NOTUNIQUE
----

Once created, these indexes are automatically used when filtering a map by key or by value:

----
ArcadeDB> SELECT FROM Movie WHERE thumbs CONTAINSKEY 'poster'
ArcadeDB> SELECT FROM Movie WHERE thumbs CONTAINSVALUE 'poster.png'
----

* Create a index for the `actors` list property  in the type `Movie`.
`BY ITEM` indexes each element of the list, so a filter on a single value uses the index:

----
ArcadeDB> CREATE INDEX ON Movie (actors BY ITEM) NOTUNIQUE
ArcadeDB> SELECT FROM Movie WHERE actors = 'Keanu Reeves'
----
...
NOTE: When a `COLLATE CI` index exists on a property, the query optimizer also recognizes queries that use `.toLowerCase()` (e.g., `WHERE Name.toLowerCase() = 'hello world'`) and will use the case-insensitive index for these queries.
...
NOTE: By default, `CREATE INDEX ... LSM_VECTOR` builds the HNSW graph immediately, so the index is ready to query as soon as the command completes. This is the recommended behavior for most use cases. If you prefer to defer graph construction to the first search (lazy mode), set `buildGraphNow: false` in the metadata.
...
The graph will be built lazily on the first `vector.neighbors()` query or when `REBUILD INDEX` is executed.
...
When `efSearch` is set in the index metadata, all queries on this index use the specified beam width by default. Individual queries can still override it by passing efSearch as the 4th argument to `vector.neighbors()`.
...
----
ArcadeDB> SELECT expand(vector.neighbors('Doc[embedding]', [0.1, 0.2, 0.3], 10))
----
...
----
ArcadeDB> SELECT expand(vector.neighbors('MultiModalRecord[imageEmbedding]', [0.1, 0.2, 0.3], 5))
----
...
NOTE: Quantization types: `NONE` (default, no compression), `INT8` (recommended, 4x compression, <2% accuracy loss), `BINARY` (32x compression, approximate search), `PRODUCT` (16-64x compression, zero-disk-I/O graph build). Quantization is transparent - vectors are stored compressed but queries work the same way.
...
ArcadeDB> CREATE INDEX ON LargeScale (embedding) LSM_VECTOR METADATA {
  ...
  maxConnections: 32,                -- Vamana per-layer degree (default); = hnswlib M=16
  beamWidth: 100,
...
* Create a sparse vector index for SPLADE / BM25 / BGE-M3 hybrid retrieval.
...
CREATE INDEX ON Doc (tokens, weights) LSM_SPARSE_VECTOR
  METADATA { dimensions: 105000, modifier: 'IDF', weightQuantization: 'FP32' }
----

The two parallel array properties carry the non-zero dimension ids and matching weights for each record. `dimensions` declares the vocabulary cap and rejects out-of-range ids at write time (set to `0` to disable the cap). `modifier: 'IDF'` enables Robertson-Sparck-Jones IDF weighting at query time; omit it (or set to `'NONE'`) for plain dot-product scoring.

`weightQuantization` controls how posting weights are stored on disk, bringing the sparse index to parity with the dense `LSM_VECTOR` index's quantization knob:

[%header,cols="1,3"]
|===
| Value | Storage / trade-off

| `INT8`
| 1 byte per weight (the default). Smallest index; scores are approximate.

| `FP16`
| 2 bytes per weight. Half the size of `FP32` with near-exact scoring.

| `FP32`
| 4 bytes per weight. Exact scoring; largest index. Choose this when approximate weights measurably hurt ranking quality.
|===

Omit `weightQuantization` to keep the `INT8` default. The setting is fixed at index-creation time; to change it, drop and recreate (or `REBUILD INDEX`) the index.

`FP16` stores weights as IEEE 754 half-precision, subnormals included, so magnitudes down to `6e-8` keep their value
rather than flushing to zero. A weight whose magnitude exceeds the format's largest finite value (`65504`) is
saturated to it rather than stored as infinity, and `NaN` is rejected at write time.

Query with `vector.sparseNeighbors`:

[source,sql]
----
SELECT expand(`vector.sparseNeighbors`('Doc[tokens,weights]', :qIdx, :qVal, 50))
----

For server-side hybrid retrieval (dense + sparse + full-text fused into one ranked top-K), see <<hybrid-search,Hybrid Search with `vector.fuse`>>.

TIP: After a large bulk load, run <<sql-compact-index,`COMPACT INDEX`>> to merge the index segments and get lower, more stable query latency without waiting for the background compaction.

For more information, see:

* <<sql-compact-index, `COMPACT INDEX`>>
...
that queries run slower during this operation. Non-idempotent commands, such as <<sql-insert-command,`INSERT`>>, <<sql-update,`UPDATE`>>,
...
TIP: To build a large `LSM_TREE` index (unique or non-unique) over data that is already loaded (single-server, no active transaction), the Java API offers a much faster offline *sorted bulk build* that constructs the index directly in compacted form. See <<index-sorted-bulk-build,Sorted bulk build>>.
...
Forces an immediate compaction of automatic indexes, merging their on-disk segments into a compacted form.

ArcadeDB compacts indexes automatically in the background. `COMPACT INDEX` lets you trigger it on demand, which is useful right after a bulk load to settle an index and get lower, more stable query latency without waiting for the background schedule. Unlike <<sql-rebuild-index,`REBUILD INDEX`>>, it does not re-index the data: it only merges the existing segments, so it is much faster.

It works for standard <<index-lsm-tree,LSM-Tree>> indexes as well as vector and sparse-vector indexes, and runs synchronously (it returns when the compaction has finished). Because it is a regular SQL command, it can be executed from any client, including the HTTP `/command` endpoint.
...
* *`&lt;index-name&gt;`* It is the index name that you want to compact.
Use `*` to compact all automatic indexes.
Quote the index name if it contains special characters like square brackets.

*Examples*

* Compact the sparse-vector index on the `tokens` and `weights` properties of the type `Doc`:

----
ArcadeDB> COMPACT INDEX `Doc[tokens,weights]`
----

* Compact all indexes:

----
ArcadeDB> COMPACT INDEX *
----
