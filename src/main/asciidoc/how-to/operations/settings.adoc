...
| `ARCADEDB_SETTINGS`     | ArcadeDB settings, as `-Darcadedb.*` arguments
| `ARCADEDB_HOME`         | ArcadeDB location
| `ARCADEDB_PID`          | `arcadedb.pid` file location
| `ARCADEDB_OPTS_MEMORY`  | JVM memory options
| `ARCADEDB_OPTS_GC`      | JVM garbage collector options (_since v26.9.1_)
|===

Use `ARCADEDB_SETTINGS` for ArcadeDB's own settings and leave `JAVA_OPTS` for JVM flags. They are
separate variables so that setting one never discards the other: this matters in containers, where the
image supplies its own values for `ARCADEDB_OPTS_GC` and `ARCADEDB_OPTS_MEMORY` and an environment
variable is replaced rather than appended to.

[source,shell]
----
$ export ARCADEDB_SETTINGS="-Darcadedb.server.rootPassword=playwithdata"
$ bin/server.sh
----
