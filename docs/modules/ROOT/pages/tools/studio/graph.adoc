...
|===
|image:studio-graph-node-menu-unselected.png[align="left",scaledwidth=30%]|Hold the selection on a node to show its context menu.
Then while still holding the selection, slide on the action to execute and then release the selection.
|image:studio-graph-node-menu-selected.png[align="right",scaledwidth=30%]
|===
...
- xref:concepts/basics.adoc#rid[Record ID (RID)]
- Type
- Properties table
...
image::openbeer-demo-custom.png[align="center"]
...
See xref:reference/graph-algorithms/algorithms.adoc#appendix-graph-algorithms[Graph Algorithms Appendix] for algorithm details, advanced usage, and the full library of graph algorithms available via Cypher and SQL.
