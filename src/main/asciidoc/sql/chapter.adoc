...
| <<in-v,inV()>>     | <<variance,variance()>> | <<distinct,distinct()>> | | | <<ifnull-function,ifnull()>>
| <<both-v,bothV()>> | <<stddev,stddev()>> | <<symmetricdifference,difference()>> | | | <<coalesce,coalesce()>>
| <<traversed-element,traversedElement()>> | <<mode,mode()>> | <<symmetricdifference,symmetricDifference()>> | | | <<uuid,uuid()>>
| <<traversed-vertex,traversedVertex()>> | <<median,median()>> | | | | <<encode,encode()>>
| <<traversed-edge,traversedEdge()>> | <<percentile,percentile()>> | | | | <<decode,decode()>>
| <<shortest-path-function,shortestPath()>> | <<random-int,randomInt()>> | | | | <<bool-and,bool_and()>>
| <<dijkstra,dijkstra()>> | | | | | <<bool-or,bool_or()>>
| <<astar,astar()>> | | | | | <<expand,expand()>>
| | | | | | <<version,version()>>
|===
