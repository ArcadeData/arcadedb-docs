...
Implement retrieval-augmented generation (RAG) that retrieves richer, more connected context for LLM queries -- all within a single database. <<graph-model,Graph traversal>> enables multi-hop entity bridging across knowledge graph relationships, <<vector-model,vector similarity>> powers semantic chunk retrieval using `vector.neighbors()` with LSM_VECTOR indexes, <<sparse-vector-search,sparse vectors>> add SPLADE / BM25-style learned sparse retrieval for exact-term and OOV matches that dense embeddings miss, <<hybrid-search,server-side hybrid fusion>> combines all of the above in one query, <<searchengine-model,full-text search>> provides keyword-based chunk lookup, and Neo4j Bolt protocol compatibility on port 7687 supports LangChain4j integration.
...
SELECT content, source, distance FROM (
  SELECT expand(vector.neighbors('Chunk[embedding]', [0.9, 0.1, 0.8, 0.2], 5))
)
----
