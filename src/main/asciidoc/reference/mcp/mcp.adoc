...
| `limit` | No | Maximum number of records to return, from 1 through 100,000 (default: 1000).
|===
...
`value` is what this server is actually running on, and `default` the built-in default.
`overridden` is true when the setting has been given an explicit value rather than inheriting its default - in the
server configuration file, by an embedding application, or at runtime with `set_server_setting`.
...
`arcadedb.server.defaultDatabases` is masked differently, because its value merely *embeds* credentials rather
than being one: the database names, user names and groups are kept, and only each password is replaced, so
`mydb[jay:hunter2:admins]` is reported as `mydb[jay:*****:admins]` _(since v26.10.1)_.
====
...
*Exit status.* The process exits non-zero on any fatal startup failure - a wrong root password, an HTTP port
already in use, an unreadable database directory - and exits `0` only when it ended the way it is meant to, with
the client closing its stdin.
This matters when a supervisor launches it (a systemd unit, a container runtime, or the MCP client itself), since
a zero status reads as "finished normally" and suppresses the restart and the alert _(since v26.10.1)_.
