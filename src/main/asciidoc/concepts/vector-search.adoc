...
then runs the next time that index is actually searched, after any reopen -- so the first query against a large,
recently-written index that was closed this way can be noticeably slower than the queries that follow it, where
`close()` used to absorb that latency instead. A session that closes and never searches the index again pays
nothing at all. Below 1,000 vectors, `close()` keeps rebuilding synchronously as before.
====
