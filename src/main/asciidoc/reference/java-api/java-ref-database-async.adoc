...
Each async worker thread batches its writes and commits them together every `arcadedb.asyncTxBatchSize` operations
(see <<arcadedb-settings,Settings>>). If that periodic commit conflicts with another worker writing the same data, it
is now retried automatically (up to `arcadedb.txRetries` attempts) instead of being silently dropped. If every retry
fails, the `onError()` callback of every command in that batch is invoked, so a genuine failure is always reported
rather than lost.

A statement that commits part-way through on its own - `UPDATE`/`DELETE`/`MOVE VERTEX ... BATCH n`, `TRUNCATE TYPE`,
`REBUILD INDEX` - makes everything batched before it durable at that moment. Since 26.10.1 the executor recognises
this, so a later conflict on the same worker never re-applies those writes and never reports them as lost.
