...
| One row per <<sql-materialized-views,materialized view>>. Properties: `name`, `query`, `backingType`, `refreshMode`, `refreshInterval`, `simpleQuery`, `lastRefreshTime`, `status`, `sourceTypes`, plus runtime metrics: `refreshCount`, `refreshTotalTimeMs`, `refreshMinTimeMs`, `refreshMaxTimeMs`, `refreshAvgTimeMs`, `errorCount`, `lastRefreshDurationMs`.
...
| `schema:triggers`
| One row per <<sql-triggers,trigger>>. Properties: `name`, `typeName`, `timing` (`BEFORE`/`AFTER`), `event` (`CREATE`/`READ`/`UPDATE`/`DELETE`), `actionType` (`SQL`/`JAVASCRIPT`/`JAVA`) and `actionCode`. Triggers whose type the current user cannot read are omitted. Available since 26.10.1.
