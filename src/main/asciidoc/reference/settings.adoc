...
|`ha.serverList`|Servers in the cluster as a list of `<host:raftPort:httpPort[:priority]>` items separated by comma. The httpPort is required for replica-to-leader HTTP forwarding. The optional priority (default 0) prefers higher-valued nodes during elections. Example: `localhost:2434:2480:10,192.168.0.1:2434:2480:0`|String|
