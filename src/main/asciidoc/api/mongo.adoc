...
In case you're running ArcadeDB with Docker, use `-e` to pass settings:

```shell
docker run --rm -p 2480:2480 -p 2424:2424 -e arcadedb.server.rootPassword=playwithdata -e arcadedb.server.plugins="MongoDB:com.arcadedb.mongo.MongoDBProtocolPlugin" arcadedata/arcadedb:latest
```
