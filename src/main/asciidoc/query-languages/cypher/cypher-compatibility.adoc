...
| Expression Evaluation | 100% | Complete expression framework with list literals
...
| Advanced Features | 40% | Named paths, OPTIONAL MATCH, WITH; no UNION
|===
...
| CALL | - | Procedure calls not supported
...
| EXISTS subquery | ~ | Basic syntax supported, variable scoping limited
|===
...
| percentileCont() | ~ | Available via SQL bridge
| stDev() | ~ | Available via SQL bridge
|===
...
| shortestPath() | ~ | Available via SQL bridge
| allShortestPaths() | ~ | Available via SQL bridge
|===
...
| Map literals | - | `RETURN {name: 'Alice'}`
...
* **CALL procedures**: Stored procedure invocation
* **FOREACH**: Iterating over lists in write operations
* **Subqueries**: Nested queries beyond EXISTS
* **List comprehensions**: `[x IN list WHERE x.age > 25 | x.name]`
* **Map projections**: `RETURN n{.name, .age}`
* **Arithmetic in RETURN**: `RETURN n.age * 2 + 10`
* **DISTINCT in RETURN**: `RETURN DISTINCT n.name`
* **EXPLAIN / PROFILE**: Query plan visualization
...
3. **Arithmetic expressions**: Expressions like `n.age * 2` in RETURN are not yet supported. Use SQL functions or pre-compute values.
