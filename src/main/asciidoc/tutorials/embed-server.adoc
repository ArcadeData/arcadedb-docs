...
<dependency>
    <groupId>com.arcadedb</groupId>
    <artifactId>arcadedb-server</artifactId>
    <version>{revnumber}-java17</version>
</dependency>
----
...
IMPORTANT: High availability is no longer part of `arcadedb-server`. Since the migration to Apache Ratis, the HA implementation lives in a separate module, `arcadedb-ha-raft`, and is discovered at startup via the Java `ServiceLoader`. If you embed only `arcadedb-server`, setting `HA_ENABLED=true` (or a non-blank `HA_SERVER_LIST`) is *not* enough: with no HA plugin on the classpath the node starts STANDALONE with no replication. Add the dependency explicitly:
...
The official server distribution and Docker image already bundle this module, so this step only applies to embedded deployments that manage their own dependencies. When HA is requested but no plugin is found, the server logs a `WARNING` at startup and records a server event under the `HA` component.
