...
* A storage class that provides `ReadWriteOnce` volumes (the cluster defaults on EKS, GKE, AKS, and DigitalOcean all work)
* (Optional) `kind` 0.24 or later and Docker for the local quickstart below
...
Raising `replicaCount` on a running HA cluster is enough: the new pods join the existing cluster on their own,
and the pods already running accept them without any change to `ha.serverList`.
This relies on the headless service publishing the address of a pod that is not Ready yet
(`publishNotReadyAddresses: true`, which the chart sets), because a pod becomes Ready only once it has joined.

NOTE: Before v26.10.1 the running pods rejected the new pod's connection, so it never joined and stayed
`NOT_READY`. If you are on an older version, upgrade or restart every pod with the enlarged `ha.serverList`.
...
[NOTE]
====
Studio and the HTTP API authenticate every request, but do not expose them directly to the public internet without an extra layer.
Place public access behind the ingress controller with TLS (as above), an OIDC/SSO authentication proxy, or a VPN, so the database is never the sole gatekeeper of a publicly reachable endpoint.
====
...
Set `limits.memory` first: _since v26.9.1_ the container image derives the JVM heap from it
(`-XX:MaxRAMPercentage=75`) unless you pin the heap yourself, so the limit is what decides how much
memory ArcadeDB actually uses. Then size the memory request to the heap you expect, so the scheduler
places the pod on a node with enough RAM.
...
|`resources.requests.memory` | (absent) | Memory request. Should equal or slightly exceed the heap to guarantee the node has enough RAM at scheduling time.
|`resources.limits.memory` | (absent) | Memory limit, and the value the heap is sized against unless you pin it: 75% of this becomes the heap, leaving the rest for off-heap buffers (e.g. `4Gi` gives a 3 Gi heap). Also prevents a single pod from consuming all node memory.
|===
...
On a High Availability cluster, set `arcadedb.server.readinessRequiresHA=true` (env `ARCADEDB_SERVER_READINESSREQUIRESHA=true`) to withhold traffic from a node until it has joined the Raft group and, once joined, until it is a member of the current Raft configuration and has caught up to within `arcadedb.server.readinessHAMaxLag` log entries (default `100`, env `ARCADEDB_SERVER_READINESSHAMAXLAG`) of the commit index. This keeps a restarted follower with a wiped or lagging log out of Service endpoints during a rolling update, so the write quorum is not dropped. The check also fails closed if the node's internal Raft engine is unhealthy, even while the server process itself is still running and responding.

This setting decides how strict the probe is about a node that is merely *behind*. It does not apply to a node whose replication has *stopped* - one that cannot write its replication log, or that hit an error it cannot apply. Such a node answers `503` whatever this setting says, because waiting for it would never help. See <<ha-node-stopped,When a node has stopped for good>> for the two fields that name the cause. _(Since v26.10.1: with this setting off, such a node used to keep answering as ready.)_
====
...
With `readOnlyRootFilesystem: true` you must also relocate the *log directory* to a writable mount, otherwise the server fails at startup with `java.nio.file.FileSystemException: ./log/arcadedb.log.0.lck: Read-only file system`.
By default the server writes logs to `./log` (relative to the working directory), which is read-only under this hardening.
Mount a writable volume (for example an `emptyDir`) and point the log directory at it by forwarding the `ARCADEDB_LOG_DIR` environment variable (it maps to the `<<arcadedb-settings,arcadedb.server.logsDirectory>>` setting):
...
volumeMounts:
  - name: arcadedb-log
    mountPath: /writable/log
----

When running an HA cluster with `readOnlyRootFilesystem: true`, also relocate the *Raft storage directory*.
By default each node creates a dynamically-named `raft-storage-<nodeName>` folder under the server root path, which is both read-only under this hardening and awkward to mount in a `StatefulSet` because the name varies per pod.
Set `<<arcadedb-settings,arcadedb.ha.raftStorageDirectory>>` to a fixed, writable path backed by a per-pod PVC (an extra `volumeClaimTemplates` entry), so the `raft-storage-*` folder is always created under the same configured path:

[source,yaml]
----
extraCommands:
  - -Darcadedb.ha.raftStorageDirectory=/var/lib/arcadedb/raft
----
====
...
====== Inspecting cluster status and testing failover

The `/api/v1/server` endpoint reports the live HA state -- current leader, replicas, and roles.
Port-forward the HTTP service and read the `ha` object:

[source,shell]
----
kubectl -n arcadedb port-forward svc/my-arcadedb-http 2480:2480 &
curl -u root:<password> http://localhost:2480/api/v1/server | jq .ha
----

To confirm automatic failover works, delete the leader pod and watch the cluster re-elect a new leader and restart the deleted pod:

[source,shell]
----
kubectl -n arcadedb delete pod my-arcadedb-0
kubectl -n arcadedb get pods -w
----

The cluster re-elects a leader within seconds; the `StatefulSet` then recreates the deleted pod, which rejoins as a replica and catches up via the Raft log.
See <<ha-concepts,High Availability>> for the underlying election and catch-up semantics.
...
For cluster sizing, three to five nodes covers most workloads; seven is the practical upper end before Raft replication overhead outweighs the added fault tolerance.
When scaling down, remove one pod at a time and never drop below the Raft majority -- losing quorum stalls writes until enough peers rejoin.
...
Direct the backup output to a volume *outside* the database PVC -- for example a dedicated `volumeClaimTemplates` entry (see <<kubernetes-values-extras,Extras>>) or an object-store target -- so a backup never competes with live data for space and survives deletion of the database volume.
In an HA cluster, take the backup at the *leader* for a consistent snapshot of committed state.

====== Observability

Enable the Prometheus plugin (`arcadedb.plugins.prometheus.enabled: true`) to expose ArcadeDB metrics on the HTTP port, then scrape them with Prometheus (see <<kubernetes-values-pod-metadata,Pod metadata>> for scrape annotations or a `PodMonitor`).
Beyond the usual host and JVM metrics, alert on:

* *Raft leader changes* -- frequent re-elections signal node instability or network problems.
* *replication lag* -- a follower falling persistently behind risks a full snapshot transfer and weakens read freshness.
* *PVC capacity* -- ArcadeDB stops accepting writes when the database volume fills; alert well before the PVC is full.
...
*Securing the Raft port with mTLS* (since `26.9.1`)::
The Raft gRPC port carries AppendEntries, RequestVote and the Ratis admin/client calls in plaintext by default, guarded only by an IP-based peer allowlist.
On a shared cluster, enable mTLS with `+arcadedb.ha.tls.*+` and pair it with a `NetworkPolicy` restricting the port to the StatefulSet's own pods.
cert-manager is the natural issuer here: a `Certificate` writes a `Secret` holding exactly the three files the settings need (`tls.crt`, `tls.key`, `ca.crt`).
Two details are easy to get wrong on Kubernetes — the certificate's SAN must be the pod's stable headless-service DNS name (`my-arcadedb-0.my-arcadedb.arcadedb.svc.cluster.local`) rather than the pod IP, and `privateKey.encoding` must be set to `PKCS8`, because cert-manager defaults to `PKCS1`, which the Raft transport does not parse.
Mount the `Secret` read-only through the chart's `volumes` / `volumeMounts` (see <<kubernetes-values-extras,Extras>>) with `defaultMode: 0400`, and point the settings at it via `arcadedb.extraCommands`.
Auto-join needs no extra configuration: the join probe inherits the server's transport settings, so it speaks TLS exactly when the cluster does.
For the manifests, the openssl equivalent, and the certificate-rotation caveat, see <<ha-grpc-mtls,mTLS for the Raft gRPC transport>>.

*Users and API tokens on a restarted pod* (since `26.10.1`)::
The server users, groups and API tokens live in the server's `config/` directory, not in a database, so a pod whose data volume is persistent but whose `config/` directory is not comes back without them.
Because it is still a member of the Raft group, it is not re-admitted, and before 26.10.1 it was never re-seeded either: it served only the users created from its own environment at bootstrap and rejected logins that worked on every other pod.
Such a pod now asks the leader for the current documents shortly after it first sees one, so no action is needed.
Persisting `config/` is still worth doing if you want a pod to come back with them already in place; see <<ha-security-documents,Cluster security documents>>.
...
[[kubernetes-ha-bootstrap]]
====== Pre-staging the database on every pod
...
. Install the chart with `replicaCount: 1` (no HA) and import the dataset.
. After import is complete, tar the database directory or take a regular full ArcadeDB backup.
. Distribute the archive into every pod's PVC. Common patterns: an init container that pulls from S3/GCS, an `nfs:` volume mount with the archive, baking the archive into the container image, or `kubectl cp` for one-off operator workflows. Each pod's PVC must contain the same database files under `arcadedb.databaseDirectory`.
. `helm upgrade` the release with `replicaCount: 3` (or more). The new pods come up with non-empty databases and the same fingerprint; the cluster elects the bootstrap source automatically and forms in seconds.

For the underlying protocol and edge cases (different-age peers, late newer joiner, cluster restart with stale data on a node), see <<ha-offline-bootstrap,Offline Cluster Bootstrap>>.
