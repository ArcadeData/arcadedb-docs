...
Build a unified customer view in a single database by leveraging xref:tutorials/multimodel.adoc#graph-model[graph traversal] for identity resolution, churn risk scoring, and cross-sell recommendations, the xref:tutorials/multimodel.adoc#document-model[document model] for fuzzy deduplication via cross-matching on customer attributes, xref:concepts/vector-search.adoc#vector-model[vector similarity] with LSM_VECTOR embeddings for customer preference and product matching, xref:tutorials/multimodel.adoc#searchengine-model[full-text search] for support ticket content indexing, and xref:concepts/timeseries.adoc#timeseries[time-series] analysis for customer journey event chains and conversion path tracking.
...
* xref:tutorials/multimodel.adoc#document-model[Document Model]
