...
```shell
docker run --rm -p 2480:2480 -p 2424:2424 --env arcadedb.server.rootPassword=playwithdata --env arcadedb.server.plugins=Redis Protocol:com.arcadedb.redisw.RedisProtocolPlugin arcadedata/arcadedb:latest
```
