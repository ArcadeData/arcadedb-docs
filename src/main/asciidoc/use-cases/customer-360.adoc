...
Build a unified customer view in a single database by leveraging <<graph-model,graph traversal>> for identity resolution, churn risk scoring, and cross-sell recommendations, the <<document-model,document model>> for fuzzy deduplication via cross-matching on customer attributes, <<vector-model,vector similarity>> with LSM_VECTOR embeddings for customer preference and product matching, <<searchengine-model,full-text search>> for support ticket content indexing, and <<timeseries,time-series>> analysis for customer journey event chains and conversion path tracking.
...
*Hybrid Preference + Ticket Search* -- _Available since v26.5.1._ Match a customer's preference vector against products while keeping support-ticket keyword evidence in the same ranked list. Useful for "next best action" surfaces that fold both behavioural similarity and recent issue context into one ranking, deduped to one product per category:
