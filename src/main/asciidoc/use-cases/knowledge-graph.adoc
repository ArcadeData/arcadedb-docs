...
*Hybrid Semantic + Keyword Search, one query* -- Fuse the semantic-similarity ranking with the abstract keyword match into a single ranked list, then collapse to one paper per topic so the literature review surface does not over-represent a single research thread:
