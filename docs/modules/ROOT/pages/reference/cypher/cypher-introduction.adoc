...
* **Advanced Pattern Matching**: Variable-length paths with xref:reference/cypher/cypher-clauses.adoc#cypher-path-mode[path modes] (WALK, TRAIL, ACYCLIC), OPTIONAL MATCH, multiple MATCH clauses
...
Connect using any Neo4j driver via the xref:how-to/connectivity/bolt.adoc#bolt-driver[BOLT protocol]:
...
You can use ArcadeDB's xref:concepts/basics.adoc#rid[RecordIDs (RID)] in Cypher queries to start from a specific vertex.
