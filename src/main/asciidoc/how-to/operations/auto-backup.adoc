...
`trigger backup`, `list backups`, `delete backup` and `restore backup` all use one definition of the backup directory,
so an archive one of them writes the others can see, whether or not the scheduler is running. The directory is
...
3. the server setting `arcadedb.server.backupDirectory`, the same directory the SQL `BACKUP DATABASE` command writes to.
