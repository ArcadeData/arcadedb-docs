...
** `LSM_SPARSE_VECTOR` posting-list inverted index for sparse-vector retrieval (BM25-style, SPLADE, BGE-M3, OpenSearch sparse). _Available since v26.5.1._
...
* Create a sparse vector index for SPLADE / BM25 / BGE-M3 hybrid retrieval. _Available since v26.5.1._
...
`weightQuantization` (since v26.7.2) controls how posting weights are stored on disk, bringing the sparse index to parity with the dense `LSM_VECTOR` index's quantization knob:
...
| `INT8`
| 1 byte per weight (the default, and the only behavior before v26.7.2). Smallest index; scores are approximate.
