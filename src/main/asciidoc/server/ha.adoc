...
To start ArcadeDB Server in High Availability (HA) mode, modify the default setting <<#_settings,`ha.enabled`>> to `true` and the server will listen for incoming connections from other nodes.
If any other servers are already started, then the current server will join the cluster.
...
Auto-discovery is enabled by default and uses TCP/IP UDP Multicast. It works only in LAN (Local Area Network) where this protocol is enabled in network routers and firewalls of your LAN.

To create a cluster outside a LAN, you have to specify the servers that are part of the cluster in terms of their ip address or host name and port.
...
You can specify a different quorum by setting the number of servers or `none` to have no quorum and `all` to wait thr response from all the servers.
