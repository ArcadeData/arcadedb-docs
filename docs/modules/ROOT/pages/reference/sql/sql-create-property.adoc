...
* Create an `EXTERNAL` property to store a vector embedding outside the primary record:

----
ArcadeDB> CREATE PROPERTY Doc.embedding ARRAY_OF_FLOATS (EXTERNAL true)
----

* Create an `EXTERNAL` property with auto-compression for large text payloads:
...
* xref:reference/sql/sql-rebuild-type.adoc#sql-rebuild-type[`REBUILD TYPE`] to migrate existing records after toggling `EXTERNAL`
