...
ask for it with `edgesSkipEntries`. _(Since v26.10.1: the first statement of every RDF file was dropped as if it
were a header, so N triples produced N-1 edges. If you were passing `edgesSkipEntries = 0` to work around that, you
can remove it.)_

Lines at the top of a source that start with `#` or `//` are treated as comments and ignored, for every text format.
_(Since v26.10.1: the comment was examined as if it were the first line of data, which could make the file be
recognised as the wrong format and fail to import.)_
...
`createdDocuments`, `createdVertices`, `createdEdges`, `errors` and `warnings`.
