[[lsm-tree]]
= LSM-Tree Algorithm
image:edit.png[link="https://github.com/ArcadeData/arcadedb-docs/blob/main/src/main/asciidoc/reference/lsm-tree.adoc" float=right]

ArcadeDB's default index algorithm is the LSM Tree. For equality-only lookups, ArcadeDB also provides a xref:concepts/indexes.adoc#hash-index[Hash Index] based on extendible hashing.
