...
ArcadeDB uses the secure Java Gremlin engine by default.
...
==== Transactions and closing a graph

Starting from version 26.9.1, closing an `ArcadeGraph` *rolls back* any transaction still open, which is the TinkerPop default. Changes that never reached `commit()` are not saved just because the graph was closed:

```java
try (final ArcadeGraph graph = ArcadeGraph.open("./databases/graph")) {
  graph.tx().begin();
  graph.addVertex("Person");
  graph.tx().commit(); // <-- WITHOUT THIS, THE VERTEX IS NOT SAVED
}
```

If you want the graph to commit when it closes, ask for it explicitly:

```java
graph.tx().onClose(Transaction.CLOSE_BEHAVIOR.COMMIT);
```

NOTE: Before 26.9.1 an open transaction was committed on close. If your application relied on that, add the `onClose()` call above.

An `ArcadeGraph` borrowed from an `ArcadeGraphFactory` pool is always returned clean: any transaction left open is rolled back before the instance goes back into the pool, so the next caller never inherits your uncommitted changes. Do not keep using an instance after calling `close()`, and note that `drop()` is not allowed on a pooled instance, because the database is shared with the rest of the pool.

==== Vertex property identifiers

Starting from version 26.9.1, `VertexProperty.id()` returns a `String` in the form `#<bucket>:<offset>-<propertyName>`, consistent with vertex and edge identifiers. It previously returned a `Long` computed from hash codes, which could come out the same for two different properties of the same vertex, making them compare as equal and disappear from results such as `properties().dedup()`.

NOTE: If your application casts or parses a vertex property id as a number, update it to handle a `String`.
...
ArcadeDB's Gremlin Server includes `ArcadeGraphManager`, a custom implementation of TinkerPop's `GraphManager` interface that provides *dynamic, on-demand registration* of databases as Gremlin graphs.
...
[[gremlin-migration-static-config]]
===== Migrating from Static Graph Configuration

If you configured graphs statically in `gremlin-server.yaml`, here are the key changes to be aware of:
...
[source,java]
----
// Use the database name directly
...
[source,shell]
----
docker run -d --name arcadeDB -p 2480:2480 -p 8182:8182 \
...
- A `hasLabel('X')` that directly follows `g.V()` or `g.E()` is answered from the type `X` *polymorphically*, like SQL's `SELECT FROM X`: it matches `X` and every type that extends it. This holds whichever way ArcadeDB executes it (a type scan, an index lookup on a property of `X` or of one of its super types, or `count()`), so `g.V().hasLabel('X').count()` always equals the number of elements `g.V().hasLabel('X')` returns. A `hasLabel()` further down a traversal, for example after `out()`, compares the element's label exactly, as TinkerPop defines it.
- A `hasLabel('bucket:<name>')` that directly follows `g.V()` or `g.E()` restricts the traversal to that one bucket, and `count()` counts that bucket.
...
If you're using Gremlin with ArcadeDB, check out https://gdotv.com/[gdotv] graph tool. It is compatible with ArcadeDB and provides a powerful visual debugger, advanced graph analytics, and much more.
