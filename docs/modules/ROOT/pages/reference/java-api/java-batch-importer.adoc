[[java-batch-importer]]
= Graph Batch
image:edit.png[link="https://github.com/ArcadeData/arcadedb-docs/blob/main/src/main/asciidoc/api-reference/java/java-batch-importer.adoc" float=right]
...
The xref:reference/http-api/http.adoc#http-batch[POST /api/v1/batch] endpoint accepts JSONL or CSV input and uses GraphBatch under the hood, with all builder options exposed as query parameters.
See xref:reference/http-api/http.adoc#http-batch[HTTP Batch Import] for full details and examples in curl, Python, and JavaScript.

== Relationship with GraphImporter

The xref:how-to/migration/graph-importer.adoc#graph-importer[GraphImporter] is a declarative, JSON-driven graph importer built on top of `GraphBatch`.
It adds file parsing (XML, CSV, JSONL), schema auto-creation, foreign key resolution, and xref:how-to/migration/graph-importer.adoc#graph-importer-post-import-commands[post-import commands].
