...
* xref:how-to/connectivity/bolt.adoc[*Neo4j BOLT*] using the official `neo4j-java-driver` -- run Cypher queries.
* xref:how-to/connectivity/postgres.adoc[*PostgreSQL wire protocol*] using any JDBC PostgreSQL driver -- run SQL.
* xref:reference/http-api/http.adoc[*HTTP/JSON API*] using `java.net.http.HttpClient` -- no driver required.
...
Add the following dependency to your Maven `pom.xml` (or the equivalent in Gradle / SBT):
...
NOTE: Most of the API works in both synchronous and asynchronous modes.
Synchronous calls execute on the current thread and return when finished -- the easiest model for procedural code.
Asynchronous calls schedule jobs onto a pool of worker threads (sized to the available cores by default), so a single-threaded application can still saturate every core.
The asynchronous API is reached via `db.async()` -- see the xref:reference/java-api/java-ref-database-async.adoc[Async Database API] page.
...
<svg class="dgm-svg" viewBox="0 0 1200 220"
     ...
     role="img" aria-label="Two Profile vertices Albert and Michelle connected by a bidirectional IsFriendOf edge.">
  <defs>
    <marker id="dgm-arrow-friends" markerWidth="10" markerHeight="10" refX="9" refY="5" orient="auto-start-reverse" markerUnits="strokeWidth">
      <path d="M0,0 L10,5 L0,10 z" class="dgm-arrowhead"/>
    </marker>
  </defs>
  <g class="dgm-circle dgm-circle-blue">
    <circle cx="300" cy="110" r="80"/>
    <text x="300" y="118" text-anchor="middle" class="dgm-title">Albert</text>
  </g>
  <g class="dgm-circle dgm-circle-purple">
    <circle cx="900" cy="110" r="80"/>
    <text x="900" y="118" text-anchor="middle" class="dgm-title">Michelle</text>
  </g>
  <line x1="380" y1="110" x2="820" y2="110" class="dgm-conn"
        marker-start="url(#dgm-arrow-friends)" marker-end="url(#dgm-arrow-friends)"/>
  <text x="600" y="95" text-anchor="middle" class="dgm-label-b">IsFriendOf</text>
</svg>
++++
...
You have basically three ways to do that (<<database-api,Java API>>, <<sql,SQL>>, https://tinkerpop.apache.org/[Apache Gremlin] and https://opencypher.org/[Open Cypher]) each one with its pros/cons:

[cols=5]
|===
|
|<<database-api,JVM Embedded API>>
|<<sql,SQL>>
...
|Remote mode|Yes (via `RemoteImmutableVertex`)|Yes|Yes (through the <<gremlin-server,Gremlin Server>> plugin)|Yes (through the <<gremlin-server,Gremlin Server>> plugin)
|===
...
====== Traverse via API (Remote)

When working with a `RemoteDatabase`, graph traversal uses the same `getVertices()` and `getEdges()` methods on the vertex object returned from a lookup.
Each call translates to a SQL query executed over HTTP -- there is no per-vertex round trip.

[source,java]
----
RemoteDatabase db = new RemoteDatabase("localhost", 2480, "school", "root", "playwithdata");

// lookupByRID returns a RemoteImmutableVertex when connected remotely
Vertex albert = db.lookupByRID(new RID(db, "#1:0"), true);

// iterate all outgoing "IsFriendOf" neighbors -- automatically paginates
for (Vertex friend : albert.getVertices(Vertex.DIRECTION.OUT, "IsFriendOf")) {
    System.out.println(friend.getString("name"));
}
----

NOTE: `getVertices()` and `getEdges()` on a remote vertex automatically paginate results in batches of 2,000 records per HTTP call (`RemoteImmutableVertex.DEFAULT_PAGE_SIZE`), so they work correctly even when the vertex has tens of thousands of neighbors without hitting the server result limit.

For a vertex with an extremely large number of edges (a _supernode_), you can tune the page size by casting to `RemoteImmutableVertex` and calling `getVerticesPaged()`:

[source,java]
----
RemoteImmutableVertex albert = (RemoteImmutableVertex) db.lookupByRID(new RID(db, "#1:0"), true);

// fetch 5,000 neighbors per HTTP call instead of the default 2,000
for (Vertex friend : albert.getVerticesPaged(Vertex.DIRECTION.OUT, 5_000, "IsFriendOf")) {
    System.out.println(friend.getString("name"));
}

// manual offset-based page (e.g. records 1001-2000)
for (Vertex friend : albert.getVertices(Vertex.DIRECTION.OUT, 1_000, 1_000, "IsFriendOf")) {
    System.out.println(friend.getString("name"));
}
----

NOTE: Each page uses a `SKIP/LIMIT` SQL clause on the server.
For supernodes with millions of edges the edge list is re-scanned from the beginning on every page (O(n^2^) total).
In that case, prefer a targeted SQL query with a server-side `WHERE` filter to narrow the result set before returning it.
...
For more information about Cypher see: <<cypher,Cypher>> support

[[remote-tutorial]]
==== Remote

When ArcadeDB runs as a separate server process, your Java application can connect over HTTP/JSON via the `RemoteDatabase` driver, which wraps the same `Database` API you use in embedded mode.

Add the network module alongside the engine:
...
Create the database on the server, then open a `RemoteDatabase` and run a query:

[source,java]
----
RemoteServer server = new RemoteServer("localhost", 2480, "root", "playwithdata");
if (!server.exists("school"))
    server.create("school");

RemoteDatabase db = new RemoteDatabase("localhost", 2480, "school", "root", "playwithdata");

db.command("sqlscript", """
    create document type Class if not exists;
    create property Class.name if not exists string;
    """);

db.command("sql",
    "insert into Class set name = :name, location = :location",
    Map.of("name", "English", "location", "3rd floor"));

ResultSet rs = db.query("sql", "select from Class");
rs.forEachRemaining(r -> System.out.println(r.toJSON()));
----

NOTE: `RemoteDatabase` is **not** thread-safe -- it carries a backend session, and a thread closing a transaction also closes the session. Don't share an instance across threads; instead create one inside the method scope where you use it.

For the full reference covering connection pooling, async calls, gRPC, and every server endpoint, see the xref:reference/java-api/java-api-remote.adoc[Remote API] and xref:reference/http-api/http.adoc[HTTP/JSON API] pages.
