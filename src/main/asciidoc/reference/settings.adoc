...
|`opencypher.loadCsv.allowFileUrls`|Allow LOAD CSV to access local files via `file:///` URLs and bare file paths. Disable for security in multi-tenant server deployments|Boolean|true
