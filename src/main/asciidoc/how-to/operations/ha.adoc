...
2. Define the list of peers in the cluster via `arcadedb.ha.serverList` (if you are deploying on Kubernetes, see <<kubernetes,Kubernetes>>).
   The value is a comma-separated list of entries in the format `host:raftPort:httpPort[:priority]`.
...
|POST
|`/api/v1/cluster/peer`
|Add a peer to the running cluster. Body: `{"peerId":"<name>","address":"host:raftPort:httpPort"}`. The new peer is seeded with the current user list so authentication works immediately.
...
|Comma-separated list of peers in the format `host:raftPort:httpPort[:priority]`. The optional priority (integer, default 0) prefers higher-valued nodes during elections. Example: `localhost:2434:2480:10,192.168.0.1:2434:2480:0`
|(empty)
