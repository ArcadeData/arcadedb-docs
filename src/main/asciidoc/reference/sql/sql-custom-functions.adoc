...
// ...or register a single method as a one-function library. A non-static method is invoked on a
// new instance of its class, created by the library through the public no-arg constructor:
database.getSchema().registerFunctionLibrary(
    new JavaMethodFunctionLibraryDefinition("math",
        com.example.MathFunctions.class.getMethod("sum", Integer.TYPE, Integer.TYPE)));

// ...or on an instance of your own, when the method needs state or dependencies the no-arg
// constructor cannot supply:
database.getSchema().registerFunctionLibrary(
    new JavaMethodFunctionLibraryDefinition("math", new com.example.MathFunctions(),
        com.example.MathFunctions.class.getMethod("sum", Integer.TYPE, Integer.TYPE)));
----
...
If a class has overloaded public methods (e.g. `format(String)` and `format(String, int)`), every overload is registered and the matching one is picked at call time by the number and, if needed, the type of the arguments passed - including varargs and standard Java numeric widening (an `int` argument matches a `long` parameter, for example). When several overloads accept the arguments, the one with the narrowest primitive parameters wins, as in Java: `twice(int)` over `twice(long)` for an `int` argument, `scale(long)` over `scale(double)` (v26.9.1). A call that no overload accepts more specifically than the others (e.g. both `format(Object)` and `format(String)` for a `String` argument, whose reference types are not ranked) is rejected as ambiguous rather than silently picking one.
