...
Look also to xref:reference/sql/sql-methods.adoc#sql-methods[ SQL Methods].

[TIP]
====
*Extended Functions Available*

ArcadeDB provides 100+ additional functions organized by namespace (`text.{asterisk}`, `map.{asterisk}`, `convert.{asterisk}`, `date.{asterisk}`, `util.{asterisk}`, `agg.{asterisk}`, etc.). These functions are available in both SQL and Cypher queries. See xref:reference/extended-functions.adoc#extended-functions[] for the complete reference.
...
See also: xref:reference/graph-algorithms/path-finding.adoc#algo-astar[Graph Algorithms: A*] for the Cypher procedure variant with geographic heuristics.
...
To know more about it, look at xref:reference/managing-dates.adoc#managing-dates[Managing Dates].
...
NOTE: The decode function returns a binary type, which can be converted to a string via xref:reference/sql/sql-methods.adoc#asstring[`asString()`].
...
See also: xref:reference/graph-algorithms/path-finding.adoc#algo-dijkstra[Graph Algorithms: Dijkstra] for the Cypher procedure variant, and xref:reference/graph-algorithms/sql-path-functions.adoc#algo-dijkstra-sql[] for the full SQL function reference in the appendix.
...
NOTE: The admissible second argument values are given xref:reference/managing-dates.adoc#time-units[here].
...
NOTE: To encode RIDs, they need to be converted to strings first via xref:reference/sql/sql-methods.adoc#asstring[`asString()`] otherwise the link target is encoded.
...
You can also use the SQL operator xref:reference/sql/sql-select.adoc#sql-select-unwind[`UNWIND`] in select to obtain the same result.
...
See also: xref:reference/graph-algorithms/sql-path-functions.adoc#algo-shortest-path-sql[Graph Algorithms: shortestPath()] for the full function reference, and xref:reference/graph-algorithms/chapter.adoc#appendix-graph-algorithms[Graph Algorithms Appendix] for a comprehensive reference of all graph algorithms.
...
NOTE: The default format is controlled by the setting xref:reference/settings.adoc#settings-sql[`arcadedb.dateFormat`].
...
Both forms work identically and can be used interchangeably. The namespaced form (`vector.*`) is recommended for new code and is documented in xref:reference/extended-functions.adoc#extended-functions[].
====
...
See xref:how-to/data-modeling/full-text-index.adoc#full-text-index-searching[] for full documentation, Lucene syntax, and examples.
...
See xref:how-to/data-modeling/full-text-index.adoc#full-text-index-searching[] for full documentation.
...
See xref:how-to/data-modeling/full-text-index.adoc#full-text-index-mlt[] for full documentation including configuration options.
...
See xref:how-to/data-modeling/full-text-index.adoc#full-text-index-mlt[] for full documentation.
