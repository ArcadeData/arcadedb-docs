...
| `groupBy`  | String | Diversifies the answer by a property: `k` becomes the number of distinct values of that property to return rather than the number of rows, and at most `groupSize` rows are returned per value. Supports nested paths (`meta.source`). Records whose property is missing share the single `null` group.
| `groupSize` | Integer | Rows to return per group when `groupBy` is set. Defaults to `1`, so the answer is the `k` nearest records with `k` distinct values. Must be `>= 1`; ignored when `groupBy` is absent.
| `maxDistance` | Number | Drops neighbors further than this distance, turning the query into a radius search bounded by `k`. The result may legitimately hold fewer than `k` rows. Defaults to no bound.
|===
...
-- One result per source document: 10 distinct sources, nearest chunk of each
SELECT vector.neighbors('Chunk[embedding]', :queryVector, 10, { groupBy: 'sourceFile' })

-- 5 distinct sources, up to 3 chunks each, none further away than 0.35
SELECT vector.neighbors(
         'Chunk[embedding]',
         :queryVector,
         5,
         { groupBy: 'sourceFile', groupSize: 3, maxDistance: 0.35 }
       )
----

[NOTE]
====
`groupBy` changes how rows are capped and nothing else about what the index can see: a grouped query is answered from
the same corpus as the ungrouped one, including vectors written since the graph was last rebuilt.

A grouped query can return fewer than `k` groups, and two index statistics tell you which of the two reasons applies:

* `groupedSearchesShortOfLimit` - the search ran out of candidate budget before reaching the `k`-th nearest group,
which happens when the nearest groups are very dense. Raising `efSearch` is what helps here.
* `groupedSearchesGroupsUnavailable` - there were no more groups to find. Usually this simply means the data holds
fewer distinct values of the `groupBy` property than you asked for: a query with `k` of 10 over a corpus with 3 source
files can only ever return 3 groups. That is expected and needs no action - raising `efSearch` cannot create a group
that does not exist, and only makes the query slower. Look into it only if you expected more groups than you got,
which can indicate a damaged index graph that `REBUILD INDEX` repairs.
====
...
* `'INT8'` - Faster than floats; preserves the top-k ordering of the true distances (the approximate
  scalar distances rank in the same order, even though their absolute values differ). Accepts a
  `vector.quantizeint8()` result or a raw int8 byte array.
* `'BINARY'` - Very fast Hamming distance, 8x fewer operations. Requires the `vector.quantizebinary()`
  result object (raw packed bits are not enough - the Hamming normalization needs the original length).
