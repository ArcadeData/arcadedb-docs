...
[discrete]
= MATCH
image:edit.png[link="https://github.com/ArcadeData/arcadedb-docs/blob/main/src/main/asciidoc/reference/cypher/cypher-clauses.adoc" float=right]
...
See xref:reference/sql/sql-custom-functions.adoc#user-functions[User Functions] for complete documentation on defining and using functions.
...
This command requires ArcadeDB to be running in **server mode** (it does not work in embedded mode). See xref:how-to/operations/users.adoc#users[Security - Users] for more information about the ArcadeDB security model.
...
NOTE: The password is stored hashed using PBKDF2 — it is never stored in plain text. See xref:how-to/operations/users.adoc#users[Security - Users] for details on password storage.
