...
The entry point it's the `xref:reference/java-api/java-ref-database-factory.adoc#databasefactory[DatabaseFactory]` class that allows to create and open a database.
...
NOTE: A `xref:reference/java-api/java-ref-database-factory.adoc#databasefactory[DatabaseFactory]` object doesn't hold the `xref:concepts/basics.adoc#database-url[Database]` instances.
It's up to you to close them once you have finished.

=== Create a new database

To create a new database from scratch, use the `.create()` method in `xref:reference/java-api/java-ref-database-factory.adoc#databasefactory[DatabaseFactory]` class.
...
You have basically three ways to do that (xref:reference/java-api/java-api-local.adoc#java-api-local[Java API], xref:reference/sql/chapter.adoc#sql[SQL], https://tinkerpop.apache.org/[Apache Gremlin] and https://opencypher.org/[Open Cypher]) each one with its pros/cons:

[cols=5]
|===
|
|xref:reference/java-api/java-api-local.adoc#java-api-local[JVM Embedded API]
...
The examples below use https://curl.se/[`curl`] for clarity. Every request is authenticated via HTTP basic authentication; in this tutorial we use the `root` user with password `arcadedb-password`.

NOTE: On Windows (PowerShell) single and double quotes inside a quoted string need to be replaced with their Unicode entity representations -- `"` (double quote) and `'` (single quote). This applies to the data argument (`-d`) of POST requests.
...
For the full reference -- every endpoint, request shape, response shape -- see the xref:reference/http-api/http.adoc#http-json-api[HTTP/JSON API] page.
