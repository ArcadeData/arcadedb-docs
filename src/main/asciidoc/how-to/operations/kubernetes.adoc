...
Mount a writable volume (for example an `emptyDir`) and point the log directory at it by forwarding the `ARCADEDB_LOG_DIR` environment variable (it maps to the `<<arcadedb-settings,arcadedb.server.logsDirectory>>` setting), available since v26.6.1:
...
Set `<<arcadedb-settings,arcadedb.ha.raftStorageDirectory>>` (available since v26.6.1) to a fixed, writable path backed by a per-pod PVC (an extra `volumeClaimTemplates` entry), so the `raft-storage-*` folder is always created under the same configured path:
