[[gremlin-api]]
= Gremlin
image:edit.png[link="https://github.com/ArcadeData/arcadedb-docs/blob/main/src/main/asciidoc/reference/gremlin/gremlin.adoc" float=right]
...
If you're using ArcadeDB as xref:tutorials/embed-server.adoc#embedded-server[embedded], please add the dependency to the `arcadedb-gremlin` library.
...
See xref:how-to/operations/gremlin-security.adoc#gremlin-security[Gremlin Security] for complete details about this vulnerability.
====
...
You can execute a Gremlin query against ArcadeDB server by using the xref:how-to/connectivity/postgres.adoc#postgres-driver[Postgres driver] and prefixing the query with `{gremlin}`.
Example:
...
You can execute a Gremlin query against ArcadeDB server by using xref:reference/http-api/http.adoc#http-json-api[HTTP/JSON API].
...
You can also override single configuration settings by using xref:reference/settings.adoc#settings-sql[ArcadeDB's settings] and prefixing the configuration key with `gremlin.`.
