...
_Available since version 26.4.1._
...
A property value must be a scalar or a list of scalars. A map value (`SET p.address = {city: 'Rome'}`), or a
right-hand side of `=` / `+=` that is neither a map nor an entity (`SET p = 5`), is rejected with a
`TypeError` and no part of the clause is written.
