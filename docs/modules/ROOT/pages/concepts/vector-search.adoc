...
* **Graph + Vectors** -- Find similar items, then traverse relationships to discover connected context (xref:use-cases/graph-rag.adoc#use-case-graph-rag[Graph RAG pattern])
* **Full-text + Vectors** -- Hybrid search combining keyword matching with semantic similarity (xref:use-cases/knowledge-graph.adoc#use-case-knowledge-graph[Knowledge Graph pattern])
* **Time Series + Vectors** -- Detect behavioral anomalies by comparing embedding patterns over time (xref:use-cases/fraud-detection.adoc#use-case-fraud-detection[Fraud Detection pattern])
...
* xref:how-to/data-modeling/vector-embeddings.adoc#vector-embeddings-howto[Vector Embeddings How-To] -- Index creation, tuning, and best practices
* xref:reference/java-api/java-vectors.adoc#java-vectors[Java Vector API] -- Programmatic vector index management
* xref:reference/sql/sql-functions.adoc#sql-functions[SQL Vector Functions] -- All 40+ vector SQL functions
