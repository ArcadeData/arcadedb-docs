[[community]]
= Community
image:edit.png[link="https://github.com/ArcadeData/arcadedb-docs/blob/main/src/main/asciidoc/appendix/community.adoc" float=right]
...
Need guaranteed response times and dedicated assistance for your production environment?
ArcadeDB offers professional support plans to help you get the most out of your deployment.

- View available plans: https://arcadedb.com/pricing.html[Pricing]
- Service level details: https://arcadedb.com/sla.html[SLA]
...
** https://en.wikipedia.org/wiki/Prepared_statement[Prepared statements] can be directly used via the xref:how-to/connectivity/postgres.adoc#postgres-driver[Postgres driver] and the xref:how-to/connectivity/jdbc.adoc#jdbc[JDBC driver].
   Furthermore, the xref:reference/java-api/java-api-local.adoc#java-api-local[Java API] and the xref:reference/http-api/http.adoc#http-json-api[HTTP/JSON API] provide parameterized queries and commands which can manually be extended to prepared statements by wrapping the respective method or request calls in a function and forwarding the function arguments to the parameterized query or command.
