...
UPDATE City    SET name = 'London', country = 'UK'
       UPSERT  WHERE name = 'London';

-- 2. Upsert the person. UNIQUE index on Account.email makes this atomic.
UPDATE Account SET name = 'Luke', email = 'luke@example.com',
                   updatedAt = sysdate()
       UPSERT  WHERE email = 'luke@example.com';

-- 3. Look up both vertices for the edge step.
LET city    = SELECT FROM City    WHERE name  = 'London';
LET account = SELECT FROM Account WHERE email = 'luke@example.com';

-- 4. Idempotently connect them. CREATE EDGE has no UPSERT clause,
...
* *Vertices* — `UPDATE ... UPSERT WHERE` either updates the existing row matched by the unique key or inserts a new one. Run the script twice with the same `email`/`name` and you still end up with exactly one `Account` and one `City`.
