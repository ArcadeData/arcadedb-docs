...
* **<<graph-model,Graph Traversal>>** -- Network topology modeling (routers, switches, links, VLANs) with path analysis
* **<<timeseries,Time Series>>** -- Interface metrics (bandwidth, latency, packet loss) with PromQL queries
* **<<vector-model,Vector Similarity>>** -- Anomaly detection on traffic pattern embeddings
