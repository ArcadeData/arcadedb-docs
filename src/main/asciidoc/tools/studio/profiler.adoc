...
Gremlin traversals are timed on the same run that returns their results, so recording does not execute a traversal twice, and a write traversal (`addV`, `addE`, ...) is still applied only once.
Their steps (`GraphStep`, `HasStep`, ...) appear in the step table and chart like SQL and Cypher steps. _(Since v26.10.1)_
