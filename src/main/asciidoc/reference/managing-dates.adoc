...
- for `DATE`: `java.time.LocalDate` (default), `java.util.Date`, `java.util.Calendar`
- for `DATETIME`: `java.time.LocalDateTime` (default), `java.util.Date`, `java.util.Calendar`, `java.time.ZonedDateTime`, `java.time.Instant`

NOTE: `java.util.Date` and `java.util.Calendar` can only handle precision down to the millisecond. To use higher precision (`DATETIME_MICROS`, `DATETIME_NANOS`) you must use the Java Time API classes such as `java.time.LocalDateTime`, `java.time.ZonedDateTime` or `java.time.Instant`.

NOTE: before 26.10.1, setting `arcadedb.dateImplementation` to `java.util.Date` or `java.util.Calendar` was not applied to `DATE` properties: values were stored as a full timestamp and read back as a `java.time.LocalDateTime`. If you used that setting, `DATE` properties written by an earlier version hold a timestamp rather than a day, and the extra time of day is still there when you read them.
...
1. ISO 8601 without a zone, e.g. `2024-02-29T13:45:10.123456`;
2. ISO 8601 with an offset or `Z`, e.g. `2024-02-29T13:45:10.123456+01:00`;
3. the database's `DATETIMEFORMAT`;
4. the database's `DATEFORMAT`;
5. the SQL timestamp spelling: an ISO date, a *space*, an optional time with an optional fractional second of any precision, and an optional offset - e.g. `2024-02-29 13:45:10.123456`, `2024-02-29 13:45:10.123456+01`, `2024-02-29 13:45`, `2024-02-29`. This is what PostgreSQL prints and what `psqlodbc` binds a timestamp as.
...
NOTE: Before 26.10.1 an unparseable date string was silently stored as `NULL` and the write reported success.
...
| [[format]]`.format()` | Method returns the date in different formats.
...
_Since v26.10.1_, a number with a fractional part is rounded down to the unit that contains it (for a
