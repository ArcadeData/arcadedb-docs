...
In batch mode the console exits with code `1` when any command failed, and `0` only when everything succeeded, so a script or CI step can rely on the exit code.
This includes a failure raised when the write is committed - a unique-index violation, a full disk - also with `SET asyncMode = true`. _(Since v26.10.1: such a failure was printed but the console still exited `0`.)_
...
Omit <pw> to be asked for the password with the echo hidden, so it is never written to
the .history file nor echoed to the output in batch mode.
----

NOTE: The `close` command can be used without path or url, then it closes the currently connected database.

NOTE: The password can be left out of `connect`, `create database`, `drop database`, `list databases` and `create user`.
The console then asks for it and hides what you type.
This is the recommended form: a password written on the command line ends up in the console's `.history` file, and in
the build log when the console runs with `-b`.
The console masks passwords in both places, but a password you never type in full cannot leak at all.
A password containing spaces is accepted.

NOTE: Long-running maintenance commands (`CHECK DATABASE`, `REBUILD INDEX`, `COMPACT INDEX`, `BACKUP DATABASE`, `IMPORT DATABASE`) render a live progress line while they run, showing the current step and percentage, e.g. `check database fix [step 3/9] Checking vertices 'Account' |========            | 42%`. The same progress is available in Studio and via the <<http-progress,HTTP progress endpoint>>.
...
NOTE: Every statement in a script run with `LOAD` must end with `;`, except optionally the very last one. A `;` is what tells `LOAD` a statement is complete, so it can also span multiple lines.
...
NOTE: The console passes backslashes to the query language untouched, so a statement means the same typed at the prompt,
passed with `-b`, or replayed with `LOAD` as it does over the HTTP API.
Escaping is the query language's business: in SQL a string literal needs two backslashes for one, so a Windows path is
written `'C:\\Users\\bob'` and a regular expression `'\\d+'`.
Before 26.9.1 the console consumed one of the two, so those statements could not be entered at all.
