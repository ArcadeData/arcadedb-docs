...
Build intelligent product and content recommendations in a single multi-model database -- no external recommendation service required. xref:tutorials/multimodel.adoc#graph-model[Graph traversal] powers collaborative filtering through relationship patterns like User-PURCHASED->Product and User-WATCHED->Show, xref:concepts/vector-search.adoc#vector-model[vector similarity] drives content-based recommendations using `vectorNeighbors()` with LSM_VECTOR indexes on product and show embeddings, and xref:concepts/timeseries.adoc#timeseries[time-series] analysis detects trending items by aggregating interaction counts over time windows.
