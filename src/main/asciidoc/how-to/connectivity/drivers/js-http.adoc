...
empty `result` (`returned: 0`, `truncated: false`). Both the driver's own README and this page
used to show `{ 1: 21 }`; it looked plausible and returned quietly, which is exactly what makes
it easy to ship. Always start positional parameter keys at `0`.
====
