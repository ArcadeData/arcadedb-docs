...
[NOTE]
====
For a *bulk* load of vertices and edges (as opposed to the occasional multi-statement upsert above), `sqlscript` is not the fastest path: each `CREATE VERTEX`/`CREATE EDGE` statement is parsed and planned individually.
The <<http-batch,`/api/v1/batch` HTTP endpoint>> and the <<java-batch-importer,`GraphBatch`>> Java API are purpose-built for this and measure several times faster on large imports.
====
...
A SQL Script supports every <<sql-commands,SQL command>> plus the following script-only statements.
...
[WARNING]
====
A retry re-runs the block from the beginning, so it is only safe while the block has published nothing yet.
A statement with an explicit batch boundary -- <<sql-update,`UPDATE`>>, <<sql-delete,`DELETE`>> or
<<sql-move-vertex,`MOVE VERTEX`>> with `BATCH <n>` -- commits as it goes, so part of the block is already stored by the
time a later statement fails, and a rollback cannot take it back.

Since 26.10.1, when that has happened the block is *not* retried: the conflict is raised to the caller, who is the
only one able to compensate for the part that is already stored. In that case the `ELSE` block does not run and
`AND CONTINUE` does not suppress the error either -- `ELSE` statements are written for a block that did nothing,
and returning an empty result would hide a conflict you need to see. A block that committed nothing before failing
is still retried the full `<n>` times, as before.

Before 26.10.1 the block was replayed, applying the already stored records a second time on every remaining
attempt, and the script could still report success.
====
...
The script uses the <<sql-update,`UPDATE ... UPSERT`>> clause for the vertices. `UPSERT` is atomic only when the `WHERE` condition matches a `UNIQUE` index, so the schema needs those indexes first (one-time setup):
