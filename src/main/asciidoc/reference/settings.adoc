...
|`ha.proxyBatchReadTimeout`|Timeout in ms a follower waits for the leader to answer a batch load it relayed via `POST /api/v1/batch`, before answering the client HTTP 503. Separate from `ha.proxyReadTimeout` because a bulk load's duration depends on the payload size, not a fixed control-plane budget.|Long|600000
...
|`ha.proxyConnectTimeout`|Connect timeout in ms for the leader proxy (replica-to-leader forwarding).|Long|5000
