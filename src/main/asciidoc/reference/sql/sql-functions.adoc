...
Every positional numeric argument is a rank. The center constant `k` (default `60`) is set only via a
trailing options map `{ k: <long> }` - a bare trailing number is always a rank, never `k`.

Syntax: `vectorRRFScore(&lt;rank&gt;, &lt;rank&gt;, ... [, { k: &lt;long&gt; }])`
