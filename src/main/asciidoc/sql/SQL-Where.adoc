...
|`+@rid+`|returns the <<RID,RID>> in the form &lt;bucket:position&gt;. It's null for embedded records. _NOTE: using `+@rid+` in a where condition slows down queries. Much better to use the <<RID,RID>> as target. Example: change `+SELECT FORM Profile WHERE @rid = #10:44+` to `+SELECT FROM #10:44+` |`+@rid = #11:0+`
