...
The <<graph-importer,GraphImporter>> is a declarative, JSON-driven graph importer built on top of `GraphBatch`.
It adds file parsing (XML, CSV, JSONL), schema auto-creation, foreign key resolution, and <<graph-importer-post-import-commands,post-import commands>>.
