...
The entries are validated before the import starts, so a missing or empty `language` or `command` is reported straight away rather than after every row has been read. _(Since v26.10.1.)_
