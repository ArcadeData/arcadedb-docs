[[sql-L[[sql-L[[sql-Insert]]E]]E]]
[discrete]

=== SQL - `INSERT`

image:../images/edit.png[link="https://github.com/ArcadeData/arcadedb-docs/blob/main/src/main/asciidoc/sql/Llink="https://github.com/ArcadeData/arcadedb-docs/blob/main/src/main/asciidoc/sql/Llink="https://github.com/ArcadeData/arcadedb-docs/blob/main/src/main/asciidoc/sql/sql-Insert.adoc"E"E" float=right]

The <<sql-L<<sql-L<<sql-Insert,E,E,`INSERT`>> command creates a new record in the database. Records can be schema-less or follow rules specified in your model.
...
NOTE: To insert only documents, vertices, or edges with a certain *unique* property, use a <<sql-L<<sql-Create-Index,E,unique index>>. 
...
* Inserts using <<sql-L<<sql-L<<sql-Select,E,E,`SELECT`>> sub-queries
...
* Inserts using <<sql-L<<sql-L<<sql-Insert,E,E,`INSERT`>> sub-queries:
...
Besides the specialized command <<sql-L<<sql-Create-Vertex,E,CREATE VERTEX>>,
...
However, edges have to be created with <<sql-L<<sql-Create-Edge,E,CREATE EDGE>>.
