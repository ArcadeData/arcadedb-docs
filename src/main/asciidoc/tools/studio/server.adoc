...
* *Executor Pools* — thread pools (active vs. allocated threads, queue depth, completed tasks, fallbacks, reclaimed).
+
Two of those columns look similar and mean opposite things.
*Caller-Run Fallbacks* counts tasks that ran on the submitting thread because the pool's queue was **full**;
sustained growth there means the pool is undersized for your workload and is the signal to raise its size.
*Reclaimed* counts tasks a waiting caller took back and ran itself because the pool was **busy** rather than full:
that is normal, healthy behaviour and needs no action.
