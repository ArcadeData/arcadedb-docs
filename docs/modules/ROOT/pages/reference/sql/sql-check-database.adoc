...
* external property buckets: scan the paired buckets used by xref:concepts/external-properties.adoc#external-properties[`EXTERNAL` properties] and detect
  orphaned blobs (records that no primary record references). Orphans can be removed automatically if the `FIX` option is enabled.
...
| `orphanedExternalRecords` | number of records found in paired external buckets that no primary record references.
                              Orphans typically result from a failed `REBUILD TYPE` or a process crash between writing
                              the external value and re-serialising the primary record.
| `orphanedExternalRecordsFixed` | number of orphaned external records actually deleted; non-zero only when `FIX` is enabled.
|===
