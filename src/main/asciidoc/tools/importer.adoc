...
| distanceFunction | | Type of distnce measure, see <<similarity,similarity measures>>.
| efConstruction |  | Size of dynamic neighbor candidate list of (during insert).
| ef |  | Number of nearest neighbors to return (in layer search).
| m |  | TODO
| vectorType | | The data type of a vector element, for example 'float'.
| idProperty | | TODO
|===
