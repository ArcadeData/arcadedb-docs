...
| `arcadedb.bolt.defaultDatabase` | (none) | Default database when not specified by the client; if unset, the first database the user has access to is used
...
| `arcadedb.bolt.maxOpenStreams` | 1024 | Maximum number of result streams one connection may keep open at the same time (see <<bolt-transactions,Transactions>>)
...
A BOLT connection can only use the databases its user has access to, as with every other ArcadeDB protocol:

* Selecting a database the user has no access to (in the session, `RUN` or `BEGIN`) fails with `Neo.ClientError.Security.Forbidden`. The same error is returned for a database that does not exist, so a user cannot discover which databases the server hosts. The default database is checked the same way.
* `SHOW DATABASES` lists only the databases the user has access to (plus the virtual `system` entry).
* `SHOW INDEXES`, `SHOW CONSTRAINTS`, `CALL db.labels()`, `db.relationshipTypes()`, `db.propertyKeys()` and `db.schema.visualization()` hide the types the user is not allowed to read.

_(Since v26.10.1; before, these commands were not filtered by the user's permissions.)_
...
Inside an explicit transaction you can run a second query before you have finished reading the results of the first
one. This happens on its own whenever the driver's fetch size is smaller than the number of rows the first query
returns, because the driver then keeps that result open while your code moves on:

[source,java]
----
// Driver configured with Config.builder().withFetchSize(1)
try (Transaction tx = session.beginTransaction()) {
    Result people = tx.run("MATCH (p:Person) RETURN p");
    people.next();                                // the first result is still open

    tx.run("MATCH (c:Company) RETURN c").consume();  // a second query in the same transaction

    while (people.hasNext()) people.next();       // finish reading the first one
    tx.commit();
}
----

Each open result holds server-side resources until you read it to the end or discard it, so a single connection may
keep at most `arcadedb.bolt.maxOpenStreams` of them open at once (1024 by default, far more than any driver uses).
...
[NOTE]
====
Temporal values (`Date`, `Time`, `LocalTime`, `DateTime`, `LocalDateTime`) are carried as native PackStream temporal structures in *both* directions. Read them with the driver's native temporal accessors — e.g. `Value.asZonedDateTime()` or `Value.asLocalDate()` — exactly as you would against Neo4j.
====
...
| `Neo.ClientError.Security.Forbidden` | an operation is not permitted for the user, including selecting a database the user has no access to (see <<bolt-access-control,Access Control>>)
...
| `Neo.ClientError.Statement.TypeError` | a value cannot be stored in a property — a map, or a list containing one. See <<cypher-compatibility,Map-Valued Properties>>
| `Neo.ClientError.Transaction.TransactionNotFound` | a transaction reference is unknown
| `Neo.ClientError.Schema.ConstraintValidationFailed` | a unique-index (constraint) violation — this is permanent, so drivers should not retry
| `Neo.ClientError.Transaction.TransactionTimedOut` | a query or transaction ran out of time
...
1. **Enable debug logging**: Start the server with `-Darcadedb.bolt.debug=true` to see detailed protocol messages. Passwords are never written to the log.
