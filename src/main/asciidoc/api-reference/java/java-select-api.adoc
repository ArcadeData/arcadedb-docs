...
* `traverseOut()` / `traverseIn()` / `traverseBoth()` -- graph traversal (see <<select-api-graph-traversal>>)
...
Use `traverseOut()`, `traverseIn()`, or `traverseBoth()` on a `SelectIterator<Vertex>` to expand to neighboring vertices.
...
`SelectTraversal` implements `Iterator<Vertex>` and provides:

* `thenOut(edgeTypes...)` -- chain another outgoing traversal
* `thenIn(edgeTypes...)` -- chain another incoming traversal
* `thenBoth(edgeTypes...)` -- chain another bidirectional traversal
