...
Practically this means, an edge type has two implicit properties:

* `@in` holding the **RID** of its source vertex,
* `@out` holding the **RID** of its destination vertex,

while a vertex type has two similarly named implicit properties:

* `@in` holding the **number** of its incoming edges,
* `@out` holding the **number** of its outgoing edges.
