...
|`ha.raftStorageDirectory`|Parent directory under which the per-node Raft storage sub-folders (`raft-storage-<nodeName>`) are created. When empty (the default), the server root path (`arcadedb.server.rootPath`) is used, preserving the existing behaviour. Set this to a writable, statically-named path (e.g. a PVC mount) on Kubernetes StatefulSets with `readOnlyRootFilesystem: true`, where the dynamically-named `raft-storage-*` directory under the read-only server root cannot be created or mounted. _(Available since v26.6.1)_|String|
