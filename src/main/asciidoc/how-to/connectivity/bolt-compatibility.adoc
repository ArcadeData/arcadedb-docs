...
| Errors
| `Neo.ClientError.*` (syntax, semantic, security, missing entity, invalid argument, constraint, type) and `Neo.TransientError.*` (drives driver retry). Since 26.10.1 a missing record, an unknown type or property, and an invalid value report their own client error codes instead of the generic `Neo.DatabaseError.General.UnknownError`
| Certified
