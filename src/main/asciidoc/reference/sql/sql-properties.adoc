...
** `IF NOT EXISTS` If specified, create the property only if not exists. The statement returns one row
either way, carrying a `created` flag: see <<sql-if-not-exists-result,the note on `IF NOT EXISTS` results>>.
If a property with the same name already exists in the type, then no error is returned

IMPORTANT: The `hidden` attribute was *removed* and there is *no plan to reimplement it*.
...
ALTER PROPERTY <type-name>.<property-name> NAME <new-property-name>
 ...
 ** `name <new-property-name>` Renames the property. This is a schema-metadata-only rename: it does not touch existing records.
    Records keep answering under the old name until they are next written, at which point they pick up the new name.
    Not allowed on an indexed property, on a column declared directly on a TIME SERIES type, or if `<new-property-name>` already exists on the type.
...
* Rename the property 'surname' to 'lastName':

----
ArcadeDB> ALTER PROPERTY User.surname NAME lastName
----
...
Anything declared on the property, such as a `DEFAULT` value or a constraint, stops applying at once: new records are created without it.

*Syntax*

[source,sql]
----
DROP PROPERTY <type>.<property> [IF EXISTS] [FORCE]

----

* *`&lt;type&gt;`* Defines the type where the property exists.
* *`&lt;property&gt;`* Defines the property you want to remove.
* *`IF EXISTS`* Prevent errors if the property does not exist when attempting to drop it. See <<sql-if-exists-result,here>> for the shape of the result.
