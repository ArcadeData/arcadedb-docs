...
| config | Map | Configuration. Optional; omitting it, or passing `{}` or `null`, all fall back to the defaults.
|===
...
| config | Map | Configuration. Optional; omitting it, or passing `{}` or `null`, all fall back to the defaults (the `overwrite` properties policy).
|===
...
|===
| Key | Type | Description
| properties | String | How to resolve a property present on both the survivor and an absorbed node: `overwrite` (the absorbed value wins; the default), `discard` (the survivor keeps its value) or `combine` (the distinct values across the merged nodes are kept, in first-seen order, as a list - or as that single value itself when every merged node agrees on it, rather than a list of duplicates). Any other value raises an error.
|===
...
// Omitting config falls back to the default 'overwrite' policy:
MATCH (a:Person {name: 'Alice'}), (b:Person {name: 'Alice'})
WHERE elementId(a) <> elementId(b)
CALL refactor.mergeNodes([a, b]) YIELD node
RETURN node
----
