...
[[sql-projections]]
= Projections
image:edit.png[link="https://github.com/ArcadeData/arcadedb-docs/blob/main/src/main/asciidoc/reference/sql/sql-projections.adoc" float=right]
...
* `&lt;expression&gt;` is an expression (see xref:reference/sql-syntax.adoc#sql-syntax[SQL Syntax]) that represents the way to calculate the value of the single projection
* `&lt;alias&gt;` is the Identifier (see xref:reference/sql-syntax.adoc#sql-syntax[SQL Syntax]) representing the field name used to return the value in the result set
...
IMPORTANT: The result of the query can be further unwound using the xref:reference/sql/sql-select.adoc#sql-select-unwind[`UNWIND`] operator.
