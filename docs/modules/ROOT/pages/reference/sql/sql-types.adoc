...
the number of buckets created by default, xref:reference/sql/sql-alter-database.adoc#sql-alter-database[`ALTER DATABASE`] command to update the `minimumbuckets` property.
...
`round-robin`. You can change the database default using the xref:reference/sql/sql-alter-database.adoc#sql-alter-database[`ALTER DATABASE`] command and the selection
