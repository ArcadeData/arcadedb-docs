...
see xref:reference/sql/sql-where.adoc#filtering[operators] and xref:reference/sql/sql-functions.adoc#sql-functions[functions]. For more information on the differences between this
implementation and the SQL-92 standard, please refer to xref:reference/sql/sql-introduction.adoc#sql-introduction[this] section.
...
* *xref:reference/sql/sql-projections.adoc#sql-projections[`Projections`]* Indicates the data you want to extract from the query as the result-set. Note: In
...
* *`FROM`* Designates the object to query. This can be a type, bucket, single xref:concepts/basics.adoc#rid[RID], set of xref:concepts/basics.adoc#rid[RID] index values sorted
...
** To query the schema you can use one of the read-only `schema:*` virtual types (see xref:reference/sql/sql-introduction.adoc#system-types[System Types] for the full list and the fields each one returns):
*** `select from schema:types` — defined types
*** `select from schema:buckets` — physical buckets
*** `select from schema:indexes` — defined indexes
*** `select from schema:database` — database metadata and settings
*** `select from schema:materializedviews` — materialized views
*** `select from schema:graphanalyticalviews` — graph analytical views
*** `select from schema:continuousaggregates` — continuous aggregates over time series
*** `select from schema:stats` — per-database runtime statistics
*** `select from schema:dictionary` — property-name dictionary
...
* *xref:reference/sql/sql-where.adoc#filtering[`WHERE`]* Designates conditions to filter the result-set.
...
* *`ORDER BY`* Designates the field with which to order the result-set. Use the optional `ASC` and `DESC` operators, or `true` (= `ASC`) and `false` (= `DESC`), to define the
 direction of the order. The default is ascending. Additionally, if you are using a xref:reference/sql/sql-projections.adoc#sql-projections[projection], you
...
* *`SKIP`* Defines the number of records you want to skip from the start of the result-set. You may find this useful in xref:reference/sql/sql-pagination.adoc#sql-pagination[Pagination], when using it in conjunction with `LIMIT`.
* *`LIMIT`* Defines the maximum number of records in the result-set. You may find this useful in xref:reference/sql/sql-pagination.adoc#sql-pagination[Pagination], when using it in conjunction with `SKIP`.
...
xref:reference/sql/sql-traverse.adoc#sql-traverse[`TRAVERSE`] on all records in the `Movie` type.
...
You can use context variables as part of a result-set in xref:reference/sql/sql-projections.adoc#sql-projections[projections]. For instance, the query below displays the
