...
`'PRETTY'`, `'PYTHON'`, `'MATLAB'`, `'JULIA'`, or `'NUMPY'` (bare comma-separated for `numpy.fromstring`).
...
(COMPACT/PYTHON/JULIA/NUMPY), space-separated (MATLAB) and multi-line (PRETTY). It also accepts an array/list
of numbers (normalized to `float[]`) and a single number (wrapped into a one-element vector).
