...
** `updateSecurity`: to update the security settings (create, modify and delete users, groups, etc.). This also gates defining a JavaScript function via `DEFINE FUNCTION ... LANGUAGE js` — see <<user-functions,User Functions>>.
** `updateSchema`: to update the database schema (create, modify and drop buckets, types and indexes, including every `ALTER TYPE` form and the `REBUILD TYPE` and `REBUILD INDEX` maintenance commands)
** `updateDatabaseSettings`: to change per-database settings — e.g. `ALTER DATABASE`, `ALTER TYPE ... BUCKETSELECTIONSTRATEGY`, materialized-view / continuous-aggregate refresh and downsampling policies.
...
NOTE: When ArcadeDB opens a `server-groups.json` written before the `updateDatabaseSettings` permission existed, it automatically migrates existing `admin` groups to include it, so upgraded clusters keep full administrative access without a manual edit.
...
[IMPORTANT]
====
Since version 26.9.1, a change to a group takes effect immediately for users that are already connected,
whether it is made through the `/api/v1/server/groups` endpoints or by editing `config/server-groups.json`.

Before 26.9.1 the database-level permissions (`updateSchema`, `updateSecurity`, `updateDatabaseSettings`)
and the `resultSetLimit` / `readTimeout` limits were read only the first time a user touched a database and
never re-read, so a revoked permission stayed usable - and a newly granted one stayed unusable - until the
server was restarted. If you have been restarting the server to apply group changes, you no longer need to.
====
