[[java-api-local]]
= Java API (Local)
image:edit.png[link="https://github.com/ArcadeData/arcadedb-docs/blob/main/src/main/asciidoc/reference/java-api/java-api-local.adoc" float=right]
...
[%header,cols=2]
|===
|xref:concepts/basics.adoc#database-url[Synchronous API]|xref:reference/java-api/java-ref-database-async.adoc#databaseasyncexecutor[Asynchronous API]
...
|The Asynchronous API schedules jobs to be executed as soon as possible by a pool of threads. ArcadeDB optimizes the usage of asynchronous threads pool to be equals to the number of cores found in the machine (you can modify it via API). Use Asynchronous API if the response of the operation can be managed in asynchronous way. Thanks to the asynchronous API, your application doesn't need to be multi-threads to use all the available cores. The asynchronous API is configurable via xref:reference/settings.adoc#setting-database[database settings].
|===
