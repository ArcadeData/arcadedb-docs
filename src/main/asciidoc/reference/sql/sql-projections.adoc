...
* `+*+` *alone*: same behavior as without `+*+`.
...
* `expand(&lt;projection&gt;)`: The result set is made of the records returned by the projection, expanded (if the projection result is a link or a collection of links) and unwinded (if the projection result is a collection or an array, one record per element). Any other value - a number, a string, a map - gives one record holding that value. A `BINARY` property is one such value: it is an opaque blob, so it is never split into one record per byte.
