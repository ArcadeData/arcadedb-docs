...
[discrete]
= SQL - `UPDATE`
image:edit.png[link="https://github.com/ArcadeData/arcadedb-docs/blob/main/src/main/asciidoc/reference/sql/sql-update.adoc" float=right]
...
The `UPSERT` clause only guarantees atomicity when you use a `UNIQUE` index and perform the look-up on the index through the xref:reference/sql/sql-where.adoc#filtering[`WHERE`] condition.
