[[studio]]
= Studio
image:edit.png[link="https://github.com/ArcadeData/arcadedb-docs/blob/main/src/main/asciidoc/tools/studio/main.adoc" float=right]

To use the ArcadeDB Studio, point your web browser to the server and port that hosts the ArcadeDB server (in "development" or "test" xref:how-to/operations/server.adoc#server-modes[mode]).
...
image::studio-command-panel-explained.png[]
...
** *Server*, monitoring and administration of the server including metrics, events, settings, and xref:tools/studio/server.adoc#studio-backup[backup configuration]
...
The result of the command will appear in the Command Result area as a xref:tools/studio/graph.adoc#studio-graph[Graph] a xref:tools/studio/table.adoc#studio-table[Table] or xref:tools/studio/json.adoc#studio-json[].
