[[filtering]]
= Filtering
image:edit.png[link="https://github.com/ArcadeData/arcadedb-docs/blob/main/src/main/asciidoc/reference/sql/sql-where.adoc" float=right]
...
|field&lt;indexes&gt;|Document field part. To know more about field part look at the full syntax: xref:reference/sql-syntax.adoc#sql-bracket[Document-API-Property]|`+WHERE tags[name='Hi'] OR tags[0...3] IN ('Hello') AND employees IS NOT NULL+`
...
| xref:reference/sql/sql-functions.adoc#sql-functions[Functions] |Any xref:reference/sql/sql-functions.adoc#sql-functions[Function] between the defined ones|`+WHERE distance(x, y, 52.20, 0.14) <= 30+`
...
|`+@rid+`|returns the xref:concepts/basics.adoc#rid[RID] in the form `+bucket:position+`. It is null for embedded records. _NOTE:_ using `+@rid+` in a "Where" condition slows down queries. Much better to use the xref:concepts/basics.adoc#rid[RID] as target. Example: change `+SELECT FROM Profile WHERE @rid = #10:44+` to `+SELECT FROM #10:44+` |`+@rid = #11:0+`
...
|string|`+NOT LIKE+`|Negation of `+LIKE+`, equivalent to `+NOT (x LIKE y)+`|`+name NOT LIKE 'Leia'+`
|string|`+NOT ILIKE+`|Negation of `+ILIKE+`, equivalent to `+NOT (x ILIKE y)+`|`+name NOT ILIKE 'lEIA'+`
...
|collection|`+CONTAINSALL+`|true if every element of the right collection is contained in the left collection|`+children.name CONTAINSALL ['Luke','Leia']+`
|collection|`+CONTAINSANY+`|true if at least one element of the right collection is contained in the left collection|`+children.name CONTAINSANY ['Jacen','Jaina','Ben']+`
...
NOTE: `+LIKE+`, `+ILIKE+`, `+MATCHES+` are not using full-text indexes.
...
Also called "Field Operators", are xref:reference/sql/sql-methods.adoc#sql-methods[SQL-Methods].
...
|`+$parent+`|Get the parent context from a sub-query. Example: `+SELECT FROM V LET $type = (TRAVERSE * FROM $parent.$current.children)+` |xref:reference/sql/sql-select.adoc#sql-select[SELECT] and xref:reference/sql/sql-traverse.adoc#sql-traverse[TRAVERSE]
|`+$current+`|Current record to use in sub-queries to refer from the parent's variable|xref:reference/sql/sql-select.adoc#sql-select[SELECT] and xref:reference/sql/sql-traverse.adoc#sql-traverse[TRAVERSE]
|`+$depth+`|The current depth of nesting|xref:reference/sql/sql-traverse.adoc#sql-traverse[TRAVERSE]
|`+$path+`|The string representation of the current path. Example: `+#6:0.in.#5:0#.out+`. You can also display it with `+SELECT $path FROM (TRAVERSE * FROM V)+`|xref:reference/sql/sql-traverse.adoc#sql-traverse[TRAVERSE]
|`+$stack+`|The List of operation in the stack. Use it to access to the history of the traversal|xref:reference/sql/sql-traverse.adoc#sql-traverse[TRAVERSE]
|`+$history+`|The set of all the records traversed as a `+Set<RID>+`|xref:reference/sql/sql-traverse.adoc#sql-traverse[TRAVERSE]
|===

To set custom variable use the xref:reference/sql/sql-script.adoc#sql-script[LET] keyword.
