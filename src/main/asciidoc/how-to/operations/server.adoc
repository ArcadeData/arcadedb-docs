[[server]]
==== Server
image:../../images/edit.png[link="https://github.com/ArcadeData/arcadedb-docs/blob/main/src/main/asciidoc/how-to/operations/server.adoc" float="right"]
...
[%header,cols=4]
|===
| Mode | Studio | Logging | WAL Flush
| `development` | Yes | Detailed | No flush (`0`)
| `test` | Yes | Brief | No flush (`0`)
| `production` | No | Brief | Auto-set to `1` if not configured
|===

The mode is controlled by the <<setting-table,`server.mode`>> setting with a default mode `development`.

In `production` mode, if `arcadedb.txWalFlush` has not been explicitly set, it is automatically changed from `0` (no flush) to `1` (flush without metadata) to ensure transaction durability on power loss.
If you explicitly set `arcadedb.txWalFlush=0` in production mode, a warning is logged.
See <<wal-flush-durability>> for details.
