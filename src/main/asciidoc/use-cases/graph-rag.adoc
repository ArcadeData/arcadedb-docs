...
Implement retrieval-augmented generation (RAG) that retrieves richer, more connected context for LLM queries -- all within a single database. <<graph-model,Graph traversal>> enables multi-hop entity bridging across knowledge graph relationships, <<vector-model,vector similarity>> powers semantic chunk retrieval using `vectorNeighbors()` with LSM_VECTOR indexes, <<searchengine-model,full-text search>> provides keyword-based chunk lookup, and Neo4j Bolt protocol compatibility on port 7687 supports LangChain4j integration.
