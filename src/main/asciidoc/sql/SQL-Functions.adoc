...
Where:
- `&lt;index&gt;` is the starting edge to retrieve. Value &ge; 0 means absolute position in the traversed stack. 0 means the first record. Negative values are counted from the end: -1 means last one, -2 means the edge before last one, etc.
