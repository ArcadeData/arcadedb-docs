...
Syntax: `SEARCH_INDEX(indexName, query)`
...
SELECT title, $score
FROM Article
WHERE SEARCH_INDEX('Article[content]', '+java +spring')
ORDER BY $score DESC
----
...
Equivalent to `SEARCH_INDEX` but without needing to know the index name.
See <<full-text-index-searching>> for full documentation.

Syntax: `SEARCH_FIELDS(fieldNames, query)`
...
Syntax: `SEARCH_INDEX_MORE(indexName, sourceRIDs [, config])`
...
Syntax: `SEARCH_FIELDS_MORE(fieldNames, sourceRIDs [, config])`
