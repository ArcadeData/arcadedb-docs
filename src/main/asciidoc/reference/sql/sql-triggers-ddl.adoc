...
Query the read-only `schema:triggers` virtual type to see the triggers a database defines. It works the same way
locally and against a remote server.
...
Each row carries `name`, `typeName`, `timing` (`BEFORE`/`AFTER`), `event` (`CREATE`/`READ`/`UPDATE`/`DELETE`),
`actionType` (`SQL`/`JAVASCRIPT`/`JAVA`) and `actionCode`. Filter and project it like any other type:

[source,sql]
----
SELECT name, timing, event FROM schema:triggers WHERE typeName = 'User'
----

Triggers defined on a type the current user cannot read are omitted rather than failing the query.

NOTE: Available since 26.10.1. See <<system-types,System Types>> for the other `schema:*` virtual types.
