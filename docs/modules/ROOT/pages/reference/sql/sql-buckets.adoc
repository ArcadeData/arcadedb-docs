...
Creates a new bucket in the database. Once created, you can use the bucket to save records by specifying its name during saves. If you want to add the new bucket to a type, follow its creation with the xref:reference/sql/sql-types.adoc#sql-alter-type[`ALTER TYPE`] command.
...
* xref:reference/sql/sql-types.adoc#sql-drop-type[`DROP TYPE`]
...
* xref:reference/sql/sql-types.adoc#sql-truncate-type[`TRUNCATE TYPE`]
