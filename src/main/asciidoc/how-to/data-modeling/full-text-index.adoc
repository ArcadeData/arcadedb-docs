...
[[full-text-index-cypher]]
====== Cypher: db.index.fulltext.queryNodes()

Full-text search is also available from Cypher, so it can be combined with graph pattern matching in the same query. This is the Neo4j-compatible equivalent of `SEARCH_INDEX()`.

[source,cypher]
----
CALL db.index.fulltext.queryNodes('Article[content]', 'java programming')
YIELD node, score
RETURN node.title AS title, score
ORDER BY score DESC
----

*Signature:* `db.index.fulltext.queryNodes(indexName, query)`, returning one row per match with `node` and `score` (BM25 relevance, higher is better).

For a full-text index declared on an edge type, use `db.index.fulltext.queryRelationships(indexName, query)`, which yields `relationship` and `score` instead.
...
Wildcard, prefix, fuzzy and regexp terms are normalized by the index's analyzer, the same way the indexed tokens were: lower-cased with the default `StandardAnalyzer`, left as they are with a case-preserving analyzer such as `WhitespaceAnalyzer` (where `Fo*` matches `Foo` but not `foo`).
...
A qualifier applies to every form of a term, phrases included: `title:"multi model"` matches the phrase in `title` only. A property indexed `BY ITEM` (for example `keywords BY ITEM` on a list) is qualified with its plain name, `keywords:graph`.
...
| `java^3`
| Boost the term `java` 3x

| `java^0.5`
| De-emphasize the term `java` (any `N < 1` lowers its weight)
...
The key names the property as the index declares it, so for a property indexed `BY ITEM` it is `"keywords by item_boost"`; the query still qualifies it with the plain name (`keywords:graph`).
