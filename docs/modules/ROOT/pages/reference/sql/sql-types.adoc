...
the number of buckets created by default, xref:reference/sql/sql-database-admin.adoc#sql-alter-database[`ALTER DATABASE`] command to update the `minimumbuckets` property.
...
See xref:concepts/basics.adoc#bucket-selection[Bucket Selection Strategy] in the concepts section for an animated explanation of how each strategy dispatches new records.

When you create a type, it inherits the bucket selection strategy defined at the database-level. By default, this is set to
`round-robin`. You can change the database default using the xref:reference/sql/sql-database-admin.adoc#sql-alter-database[`ALTER DATABASE`] command and the selection
...
* xref:reference/sql/sql-buckets.adoc#sql-create-bucket[`CREATE BUCKET`]
...
* xref:reference/sql/sql-buckets.adoc#sql-truncate-bucket[`TRUNCATE BUCKET`]
* <<sql-create-type,`CREATE TYPE`>>
