...
|<<_asdouble,asDouble()>>|<<_right,right()>>|<<_transform,transform()>>| |<<_ifempty,ifempty()>>
|<<_asfloat,asFloat()>>|<<_prefix,prefix()>>|<<_field,field()>>| |<<_ifnull-method,ifnull()>>
|<<_asinteger,asInteger()>>|<<_trim,trim()>>|<<_sort,sort()>>| |<<_precision,precision()>>
