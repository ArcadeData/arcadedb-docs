...
`FULL_TEXT` is an Apache Lucene index tokenized and analyzed per field. Use it when you need a search experience over natural-language text — tokenization, stemming, fuzzy match, "more like this" recommendations, relevance ranking — rather than the literal byte equality that LSM and Hash give you.

Highlights:

* Pluggable *analyzer* per field (standard, stop-word, language-specific, custom).
* Full Lucene query syntax: phrase, proximity, fuzzy (`~`), wildcards, boolean operators.
