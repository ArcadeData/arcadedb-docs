...
| <<_inv,inV()>>     | <<_count,count()>> | <<_unionall,unionall()>> |  <<_if,if()>>
...
| <<_traversedvertex,traversedVertex()>> | <<_percentile,percentile()>> | <<_encode_,encode()>> |
| <<_traversededge,traversedEdge()>> | <<_variance,variance()>> | <<_decode_,decode()>> |
| <<_shortestpath,shortestPath()>> | <<_stddev,stddev()>>| |
| <<_dijkstra,dijkstra()>> | | |
| <<_astar,astar()>> | | |
|===
