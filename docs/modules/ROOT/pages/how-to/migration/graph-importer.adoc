[[graph-importer]]
= Graph Importer
image:edit.png[link="https://github.com/ArcadeData/arcadedb-docs/blob/main/src/main/asciidoc/how-to/migration/graph-importer.adoc" float=right]
...
2. **Pass 2** -- Create all edges from the in-memory topology using xref:reference/java-api/java-batch-importer.adoc#java-batch-importer[GraphBatch], one batch per edge type with bidirectional edges for full IN+OUT traversal
...
If any post-import command triggers an asynchronous xref:how-to/data-modeling/graph-olap.adoc#graph-olap[Graph Analytical View] build, the importer automatically waits (up to 10 minutes) for all views to reach READY status before returning.
