...
NOTE: A `xref:reference/java-api/java-ref-database.adoc#databasefactory[DatabaseFactory]` object doesn't hold the `xref:concepts/basics.adoc#database-url[Database]` instances.
...
The `xref:concepts/basics.adoc#database-url[Database]` instance extends Java7 `AutoClosable` interface, that means the database is closed automatically when the Database variable reaches out of the scope.
