...
*Syntax:* `vector.rrfscore(<rank1>, <rank2>, ..., [{ k: <long> }])`

Every positional numeric argument is treated as a rank. The center constant `k` (default 60) is set only
via the trailing options map `{ k: <long> }` - a bare trailing number is always a rank, never `k`.
