...
Manage identity access across complex permission hierarchies while detecting security anomalies and maintaining compliance. <<graph-model,Graph traversal>> resolves permissions through nested group/role hierarchies via SQL MATCH queries, <<vector-model,vector similarity>> detects behavioral anomalies using access pattern embeddings with `vector.neighbors()`, <<timeseries,time-series>> tracking captures access audit logs for SOX/GDPR compliance reporting, and the <<postgres-driver,PostgreSQL wire protocol>> provides Python connectivity for integration.
...
SELECT email, distance FROM (
  SELECT expand(vector.neighbors('Identity[access_pattern_vec]', [0.1,0.9,0.1,0.8,0.2,0.7,0.3,0.6], 3))
) WHERE distance > 0.5
----
