...
. For time series data visualization, use ArcadeDB's native xref:concepts/timeseries.adoc#timeseries[Grafana integration] endpoints with the Infinity datasource plugin
...
* xref:reference/settings.adoc#arcadedb-settings[Server Settings] -- All configuration parameters
* xref:concepts/timeseries.adoc#timeseries[Time Series] -- Grafana DataFrame endpoints for time series dashboards
* xref:use-cases/realtime-analytics.adoc#use-case-realtime-analytics[Realtime Analytics Use Case] -- Complete monitoring example with Grafana
