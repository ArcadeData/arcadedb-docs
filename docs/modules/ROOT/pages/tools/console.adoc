[[console]]
= Console
image:edit.png[link="https://github.com/ArcadeData/arcadedb-docs/blob/main/src/main/asciidoc/tools/console.adoc" float=right]
...
* `-D` allows to pass xref:reference/settings.adoc#settings-sql[settings],
...
NOTE: The console does line-by-line processing, so commands and queries need to be in a single line; however, a line break can be escaped by `\`.
...
NOTE: Other xref:reference/settings.adoc#setting-table[settings] can be changed this way, too.
...
NOTE: Make sure to `create database` or `connect` to a database first in the script before using xref:reference/sql/chapter.adoc#sql[SQL commands].
