...
ArcadeDB provides 100+ additional functions organized by namespace (`text.*`, `map.*`, `convert.*`, `date.*`, `util.*`, `agg.*`, etc.). These functions are available in both SQL and Cypher queries. See <<extended-functions>> for the complete reference.
