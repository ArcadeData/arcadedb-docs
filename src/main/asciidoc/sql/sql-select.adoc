[[sql-L[[sql-L[[sql-Select]]E]]E]]
[discrete]
=== SQL - `SELECT`

image:../images/edit.png[link="https://github.com/ArcadeData/arcadedb-docs/blob/main/src/main/asciidoc/sql/Llink="https://github.com/ArcadeData/arcadedb-docs/blob/main/src/main/asciidoc/sql/Llink="https://github.com/ArcadeData/arcadedb-docs/blob/main/src/main/asciidoc/sql/sql-Select.adoc"E"E" float=right]

ArcadeDB supports the SQL language to execute queries against the database engine. For more information,
see <<Filtering,operators>> and <<sql-L<<sql-L<<sql-Functions,E,E,functions>>. For more information on the differences between this
implementation and the SQL-92 standard, please refer to <<sql-L<<sql-L<<sql-Introduction,E,E,this>> section.
...
* *<<sql-L<<sql-L<<sql-Projections,E,E,`Projections`>>* Indicates the data you want to extract from the query as the result-set. Note: In
...
* *<<sql-L<<sql-LET,E,`LET`>>* Binds context variables to use in projections, conditions or sub-queries.
...
* *`ORDER BY`* Designates the field with which to order the result-set. Use the optional `ASC` and `DESC` operators to define the
 direction of the order. The default is ascending. Additionally, if you are using a <<sql-L<<sql-L<<sql-Projections,E,E,projection>>, you
...
* *`SKIP`* Defines the number of records you want to skip from the start of the result-set. You may find this useful in <<sql-L<<sql-L<<sql-Pagination,E,E,Pagination>>, when using it in conjunction with `LIMIT`.
* *`LIMIT`* Defines the maximum number of records in the result-set. You may find this useful in <<sql-L<<sql-L<<sql-Pagination,E,E,Pagination>>, when using it in conjunction with `SKIP`.
...
[[sql-L[[sql-Select-Projections]]E]]
**Projections **
...
<<sql-L<<sql-L<<sql-Traverse,E,E,`TRAVERSE`>> on all records in the `Movie` type.
...
You can use context variables as part of a result-set in <<sql-L<<sql-L<<sql-Projections,E,E,projections>>. For instance, the query below displays the
