...
A `BINARY` property is announced over the wire as PostgreSQL's `bytea` type (with the standard `\x<hex>` text
encoding), so `ResultSet.getBytes()`/`PreparedStatement.setBytes()` round-trip it as raw bytes.
