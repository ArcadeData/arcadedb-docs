[[http-json-api]]
= HTTP/JSON API
image:edit.png[link="https://github.com/ArcadeData/arcadedb-docs/blob/main/src/main/asciidoc/reference/http-api/http.adoc" float=right]
...
See xref:reference/settings.adoc#settings-sql[`server.httpAuthSessionExpireTimeout`] to configure this timeout.
...
After a period of inactivity (default is 30 seconds, see xref:reference/settings.adoc#settings-sql[`server.httpTxExpireTimeout`]), the server automatically rolls back and purges expired transactions.

[discrete]
== Transaction Scripts

In case xref:reference/sql/chapter.adoc#sql[SQL] (`sql`) is supposed to be used as language for a transactions, the language variant xref:reference/sql/sql-script.adoc#sql-script[SQL Script] (`sqlscript`) is also available.
...
* `set server setting <key> <value>` sets the server setting with `key` to `value`, see the xref:reference/settings.adoc#setting-table[list of server-level settings]
* `set database setting <dbname> <key> <value>` sets the database's <dbname> with `key` to `value`, see the xref:reference/settings.adoc#setting-table[list of database-level settings]
...
* `align database <dbname>` aligns database `<dbname>`, see the associated xref:reference/sql/sql-align-database.adoc#sql-align-database[SQL command]
* `restore database <dbname> <url>` restores a database from a backup file. The URL can be a local path (`file://`), HTTP/HTTPS URL, or classpath resource. The database must not already exist. See xref:how-to/operations/restore.adoc#restore[].
...
This serializer is used by xref:tools/studio/main.adoc#studio[Studio].

NOTE: Asynchronous commands (using `"awaitResponse": false`) are queued, and their processing is configurable via xref:reference/settings.adoc#setting-database[`async*` database settings].
...
High-performance bulk import of vertices and edges using the xref:reference/java-api/java-batch-importer.adoc#java-batch-importer[GraphBatch] engine under the hood.
...
All parameters are optional and map directly to xref:reference/java-api/java-batch-importer.adoc#java-batch-importer[GraphBatch] builder options:
