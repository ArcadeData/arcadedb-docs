...
GraphBatch is also available over HTTP for non-Java clients.
The <<http-batch,POST /api/v1/batch>> endpoint accepts JSONL or CSV input and uses GraphBatch under the hood, with all builder options exposed as query parameters.
See <<http-batch,HTTP Batch Import>> for full details and examples in curl, Python, and JavaScript.
...
[cols="2,3"]
|===
| Old (GraphImporter) | New (GraphBatch)

| `new GraphImporter(db, vertices, edges, Type.LONG)`
| `db.batch().withExpectedEdgeCount(edges).build()`

| `importer.createVertex("Person", id, props)`
| `batch.createVertices("Person", count)` (returns RID array)
...
| `batch.newEdge(srcRID, "KNOWS", dstRID, ...)`

| `importer.close()`
| `batch.close()` (or use try-with-resources)
|===
