...
SELECT [ [DISTINCT] <Projections> ]
    [ FROM <Target> [ [AS] <Alias> ] [ LET <Assignment>* ] ]
    [ WHERE <Condition>* ]
    [ GROUP BY <Field>* ]
...
** To query the schema you can use one of the read-only `schema:*` virtual types (see <<system-types,System Types>> for the full list and the fields each one returns):
...
*** `select from schema:triggers` — defined triggers
*** `select from schema:stats` — per-database runtime statistics
*** `select from schema:dictionary` — property-name dictionary
+
The `schema:*` targets support `WHERE`, `ORDER BY`, `SKIP` and `LIMIT` like any other target.
...
* *`AS <Alias>`* Optionally names the records the target returns, so the rest of the statement can refer to them by
 that name. The `AS` keyword is optional: `FROM Person p` and `FROM Person AS p` are the same. The alias can be used in
 the projections, `WHERE`, `LET`, `GROUP BY` and `ORDER BY`, and in `UPDATE` and `DELETE`, which take the same `FROM`
 clause. `p.name` is the record's `name` property and a bare `p` is the record itself, so `SELECT p FROM Person p` and
 `SELECT FROM Person` return the same thing. Qualifying a property with the alias does not cost you an index:
 `SELECT FROM Person p WHERE p.name = 'Luke'` uses an index on `Person.name` exactly as the unqualified form does.
 When a projection gives a column the same name as the target alias, `ORDER BY` and `GROUP BY` mean the column.
...
* The same query, naming the target `p` and referring to it in the filter:
[source,sql]
----
ArcadeDB> SELECT FROM Person p WHERE p.name LIKE 'Luk%'
----
...
`UNWIND` works the same way on the array types (`SHORT[]`, `INTEGER[]`, `LONG[]`, `FLOAT[]`, `DOUBLE[]`), giving one record per element. A `BINARY` property is left untouched: it is an opaque blob, not a sequence of values, so it is never split into one record per byte.
