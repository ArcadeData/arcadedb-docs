...
* ArcadeDB running (xref:how-to/operations/docker.adoc#quick-start-docker[Docker] or binary install)
* A way to send queries (xref:tools/console.adoc#console-tutorial[Console], xref:reference/http-api/http.adoc#http-json-api[HTTP API], or a xref:tutorials/python-quickstart.adoc#python-quickstart[Python]/xref:tutorials/javascript-quickstart.adoc#javascript-quickstart[JavaScript] client)
...
NOTE: In production, embeddings are typically 384-1536 dimensions. This tutorial uses 4 dimensions for simplicity. For production workloads, add `quantization: 'INT8'` for significantly better search performance -- see xref:concepts/vector-search.adoc#quantization-performance[] below.
...
This two-step pattern -- vector search to find semantically similar items, then graph traversal to expand through relationships -- is the foundation of the xref:use-cases/graph-rag.adoc#use-case-graph-rag[Graph RAG] and xref:use-cases/recommendation-engine.adoc#use-case-recommendation-engine[Recommendation Engine] patterns.
...
See xref:concepts/vector-search.adoc#adaptive-efsearch[Adaptive efSearch] for details on how the default strategy works.
...
* xref:concepts/vector-search.adoc#vector-search-concepts[Vector Search Concepts] -- Architecture, algorithms, and parameter tuning
* xref:how-to/data-modeling/vector-embeddings.adoc#vector-embeddings-howto[Vector Embeddings How-To] -- Production best practices
* xref:use-cases/recommendation-engine.adoc#use-case-recommendation-engine[Recommendation Engine] -- Full use case with vector + graph + time-series
* xref:use-cases/graph-rag.adoc#use-case-graph-rag[Graph RAG] -- Vector + graph for LLM retrieval augmentation
