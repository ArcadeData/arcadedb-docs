...
NOTE: A full scan of a type or bucket (`iterateType()`, `iterateBucket()`, or a query that scans a type) fires the read listeners when it reads each record, once per record, even for records your code then never reads. A record a before-read or after-read listener filters out is not returned by the scan _(since v26.10.1)_.
