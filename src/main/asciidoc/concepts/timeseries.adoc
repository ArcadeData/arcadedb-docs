...
the column order is what an export records and what a restore reads back. `CREATE TIMESERIES TYPE` has one slot for
the timestamp, one for `TAGS` and one for `FIELDS`, so a type created this way always has that order.

The Java schema builder accepts any order, including a field declared before a tag, and keeps it. If you then
render that builder as SQL - which is what the *remote* schema builder does - the statement groups the columns into
the three slots, so the type it creates can carry the same columns in a different order than the embedded one
would. Declare your columns timestamp-first, then tags, then fields if you want the two to be identical, and note
that importing a JSONL export of a type whose columns are not in that order into a remote database is refused
rather than silently loading the values into the wrong columns.
