...
The storage shape is declared on the edge *type*, so every way of creating an edge -- SQL, Cypher, Gremlin, the Java API, `GraphBatch`, the async executor -- produces the shape the schema asks for.
...
* you mostly traverse *from vertices* rather than querying the edge type directly.

An edge that needs a weight, a timestamp or any other attribute must be a regular edge.

Querying the edge type directly does work -- `SELECT FROM <EdgeType>` returns the edges -- but it costs more than on a regular type, because there are no records to scan and no index to consult: the query reads the vertices instead. If that is your main access pattern, prefer a regular edge type. See <<lightweight-edges-querying,Querying a lightweight edge type>>.
...
* *No records, therefore no usable RID.* Every lightweight edge of a type reports the same placeholder `@rid`, so you cannot address one by RID. See <<lightweight-edges-querying,Querying a lightweight edge type>> for what this costs.
...
[[lightweight-edges-querying]]
==== Querying a lightweight edge type

These all work and agree with each other:
...
Because a lightweight edge has no record, none of them can read the edge type's own storage: they read the vertices that hold the edges. Two consequences are worth knowing before you rely on them:

* *`count(*)` is not instant.* On a regular type it answers from a stored counter; on a lightweight type no such counter can be correct, so the count reads the graph. A throttled `WARNING` in the server log tells you when this happens on a large database.
* *`LIMIT` does not make it cheap.* There is nothing to seek into, so a small `LIMIT` may still read a large part of the vertex set.

Anchoring the query on the vertices stays the cheapest way to reach these edges, and is unchanged:

[source,sql]
----
SELECT expand(outE('Follows')) FROM Person
SELECT FROM Follows WHERE @out = #12:0
----

`TRAVERSE ... FROM <EdgeType>` and `TRUNCATE TYPE <EdgeType>` work the same way, reading the vertices behind the scenes:

[source,sql]
----
TRAVERSE in, out FROM Follows
TRUNCATE TYPE Follows
----

[NOTE]
====
`TRUNCATE TYPE` on a lightweight edge type with a subtype needs `POLYMORPHIC` (e.g. `TRUNCATE TYPE Follows POLYMORPHIC`); without it, the statement refuses rather than guessing whether you meant the subtype's edges too.
A lightweight edge has no records of its own, so there is no way to scope the command to that type alone.

This applies to the type you name, not to the whole hierarchy: a regular edge type that merely has a lightweight
subtype below it truncates normally, clearing its own records and leaving the subtype's edges untouched, as it would
with any other subtype. _Since v26.10.1_ - earlier versions refused that case too.
====
...
The shape used to be chosen per call, with `Vertex.newLightEdge()` and the `withLightEdges` flag on `GraphBatch`, the remote batch builder, the HTTP batch endpoint and the async executor. Those all still work and are deprecated.
