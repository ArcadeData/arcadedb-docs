...
| <<_bothv,bothV()>> | <<_mode,mode()>>    | <<_flatten,flatten()>> | <<_expand,expand()>>
| <<_traversedelement,traversedElement()>> | <<_median,median()>>  | <<_last,last()>> | <<_strcmpci,strcmpci()>>
| <<_traversedvertex,traversedVertex()>> | <<_percentile,percentile()>> | <<_symmetricDifference,symmetricDifference()>> |
| <<_traversededge,traversedEdge()>> | <<_variance,variance()>> | | 
