...
the same corpus as the ungrouped one, including vectors written since the graph was last rebuilt. Grouping is
best-effort in one respect only - an index whose nearest group is denser than its candidate budget returns fewer than
`k` groups and counts the query in the `groupedSearchesShortOfLimit` index statistic, which is the signal to raise
`efSearch`.
====
