...
The Importer takes additional settings as pairs of setting name and value, where the setting name is prefixed with minus (`-`): `-<setting-name> <setting-value>`.

Example:

```
~/arcadedb $ import database -url file.csv -forceDatabaseCreate true
```
