...
earlier version are read as they are: a block whose recorded sum was poisoned is decompressed when a `SUM` or `AVG`
needs it, and the next compaction, downsampling or retention rewrite of the block records both statistics. The
