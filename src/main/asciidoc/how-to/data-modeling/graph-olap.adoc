...
A transaction that holds uncommitted changes does not use the view: the view serves the committed graph only, and a
query inside the transaction must see the transaction's own writes. Such a query runs on the records until the
transaction commits.
