...
```shell
docker run --rm -p 2480:2480 -p 2424:2424 --env arcadedb.server.rootPassword=playwithdata --env arcadedb.server.plugins=MongoDB Protocol:com.arcadedb.mongo.MongoDBProtocolPlugin arcadedata/arcadedb:latest
```
