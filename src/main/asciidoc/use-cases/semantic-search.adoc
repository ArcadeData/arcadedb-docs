...
* **<<sparse-vector-search,Sparse Vector Retrieval>>** -- SPLADE / BM25 / BGE-M3 sparse embeddings via `LSM_SPARSE_VECTOR`
* **<<hybrid-search,Server-side Hybrid Fusion>>** -- `vector.fuse` combining dense + sparse + full-text in one query with RRF / DBSF / LINEAR strategies
