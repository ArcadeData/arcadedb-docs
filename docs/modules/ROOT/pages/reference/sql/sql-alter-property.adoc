 ...
 ** `external <true|false>` If true, the property's value is stored in a paired external bucket instead of inline in the primary record.
   See xref:concepts/external-properties.adoc#external-properties[External Properties].
   When toggled on a type that already contains records, run xref:reference/sql/sql-rebuild-type.adoc#sql-rebuild-type[`REBUILD TYPE`] to migrate the existing records to the new layout.
 ** `compression <none|fast|max|auto>` Compression policy for an `EXTERNAL` property's stored value.
    ...
    `none` (default) stores the value raw, `fast` uses the LZ4 fast encoder, `max` uses LZ4 HC for smaller files at the cost of write CPU, `auto` tries `fast` and keeps it only when it saves more than 10% of bytes.
...
* Promote an existing property to `EXTERNAL` storage and re-serialise the records:
...
* Change the compression policy of an `EXTERNAL` property:

----
ArcadeDB> ALTER PROPERTY Doc.body COMPRESSION 'max'
----
...
* xref:reference/sql/sql-alter-type.adoc#sql-alter-type[`ALTER TYPE`]
