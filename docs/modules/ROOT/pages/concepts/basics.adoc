= Record
image:edit.png[link="https://github.com/ArcadeData/arcadedb-docs/blob/main/src/main/asciidoc/concepts/basics.adoc" float=right]
...
Practically, this is done by xref:reference/sql/sql-create-type.adoc#sql-create-type[extending a type] or xref:reference/sql/sql-alter-type.adoc#sql-alter-type[setting a super-type].
...
When you create a new record, ArcadeDB selects the bucket to store it in using a xref:reference/sql/sql-create-type.adoc#bucket-selection[configurable strategy].
...
When you create a new type, the xref:reference/sql/sql-create-type.adoc#sql-create-type[`CREATE TYPE`] statement automatically creates the physical buckets (files) that serve as the default location in which to store data for that type.
...
Use xref:reference/sql/sql-create-type.adoc#sql-create-type[`CREATE DOCUMENT TYPE`] instead.
