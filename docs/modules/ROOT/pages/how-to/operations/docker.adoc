...
The default backup directory is `/home/arcadedb/backups`. You can customize it with `-Darcadedb.server.backupDirectory=/mybackup`. Remember to execute regular xref:reference/sql/sql-database-admin.adoc#sql-backup-database[`BACKUP DATABASE`] operations.
