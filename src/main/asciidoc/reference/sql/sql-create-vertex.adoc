...
`CREATE VERTEX` does not have an `UPSERT` clause. To create a vertex *only if it does not already exist*, or update it otherwise, use the <<sql-update,`UPDATE ... UPSERT`>> command against the vertex type. When the target type is a vertex type, `UPSERT` creates a real vertex (not a plain document), so it can be used as the endpoint of an edge right away.
...
Re-running the same `UPDATE ... UPSERT` updates the existing vertex instead of creating a duplicate. See <<sql-update,`UPDATE`>> for the full syntax and the upsert limitations.
