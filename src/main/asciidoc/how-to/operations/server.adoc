...
| `development` | Yes | Detailed | No flush (`0`) | Enabled
| `test` | Yes | Brief | No flush (`0`) | Enabled
| `production` | No (can be force-enabled) | Brief | Auto-set to `1` if not configured | Auto-disabled if not configured
|===

The mode is controlled by the <<setting-table,`server.mode`>> setting with a default mode `development`.

In `production` mode the Studio web tool is not served by default, to reduce the attack surface. If you need it (for example to inspect a database running in production), force-enable it with the <<setting-table,`studio.enabled`>> setting:

[source,shell]
----
$ bin/server.sh -Darcadedb.server.mode=production -Darcadedb.studio.enabled=true
----

When enabling Studio in production, restrict network access to the HTTP port (for example with a firewall, a reverse proxy, or by enabling SSL), since it exposes a web UI on the public-facing server.
...
* **WAL flush** (`arcadedb.txWalFlush`): set to `1` (flush without metadata) for transaction durability. If you explicitly set it to `0`, a warning is logged. See <<wal-flush-durability>> for details.
...
* **Studio web tool** (`arcadedb.studio.enabled`): the Studio web UI is not served. Set it to `true` to force-enable Studio in production (see <<server-modes>>).
...
INFO  - Studio web tool: disabled [OK]
----
...
A database entry may be followed by `{restore:<URL>}`, which restores that database from a backup archive at start-up, replacing
any copy already on disk (e.g. `-Darcadedb.server.defaultDatabases=Universe[albert:einstein]{restore:/tmp/universe.zip}`).

[WARNING]
====
In a cluster, `restore:` acts on the node that is starting and on that node only: the other nodes keep their current copy of the
database and are not told anything. This is intentional - it is per-node boot configuration, so a rolling restart does not wipe
the database cluster-wide - but it does mean a node can come back with different content than its peers. Since 26.10.1 the server
logs a WARNING naming the database whenever this replaces a replicated one, so the divergence is visible in the log.
====
...
[TIP]
====
A common use of a Server Plugin is to register **custom Java functions** on a standalone server. Since there is no auto-discovery, drop your JAR in `lib/` (or `lib/plugins/`), call `database.getSchema().registerFunctionLibrary(...)` from the plugin's `startService()` method, and enable the plugin via `arcadedb.server.plugins`. See <<user-functions-java,User Functions in Java>> for a complete example.
====
