...
NOTE: `like()` and `ilike()` follow exactly the same rules as SQL `LIKE`/`ILIKE`: a record without the property, or a `null` pattern, does not match; a non-string value is matched by its text form; a list property matches when any of its items does. Before v26.10.1 `ilike()` threw a `NullPointerException` on a record without the property, and both failed on non-string values.

NOTE: A comparison against a missing or `null` value is never true. `lt()`, `le()`, `gt()`, `ge()` and `between()` all skip a record that does not carry the property, or carries it as `null`, exactly as SQL does - so `age < 30` never matches a record without an `age`. Use `isNull()` / `isNotNull()` to ask about absent values. Before v26.10.1 `lt()` and `le()` treated a missing value as smaller than any value and matched such records, and they did so only while no index existed on the property - so the same query over the same data could return a different number of rows depending on whether an index was present.
...
When the ORDER BY property matches an indexed property used in the WHERE condition, ArcadeDB reads results directly from the index in sorted order, avoiding an in-memory sort. This also applies when ORDER BY targets the property immediately following a matched composite-index prefix (see <<select-api-index-detection,Index Detection>>): the index is scanned as a directional range in that case, so it already returns rows in the requested order.
...
`json()` and `json(JSONObject)` are inverse: reading a serialized query back and serializing it again produces the
same document.

The `where` entry is a condition, and a condition is one of:
...
| `[left, "<operator>", right]`
| A binary condition, e.g. `[":name", "=", "John"]`.

| `[left, "<operator>"]`
| A unary condition: `not`, `is null` and `is not null` are the only operators that take no right operand.

| `[condition]`
| A single condition, the shape a query with one `where` leaf serializes to.
|===

An operand is `":property"` for a property, `"#parameter"` for a named parameter, a nested condition array, or a
literal value. A nested array **is a parenthesis**: it is evaluated as one unit, so
`[[":a", "=", 2], "and", [[":b", "=", 1], "or", [":b", "=", 3]]]` means `a = 2 AND (b = 1 OR b = 3)` and never the
re-associated `(a = 2 AND b = 1) OR b = 3`. This is the only way to hand the native API an explicitly grouped
condition: the fluent builder appends conditions left to right and applies operator precedence.

A literal value that itself starts with `:` or `#` is escaped by doubling that leading character (e.g. the literal
`":alice"` is written as `"::alice"`), so it is never confused with a property or parameter reference. This
happens automatically on serialization; a hand-written literal starting with `:` or `#` must be doubled the same
way.

The right operand of `between` and `in` is a plain array of values -- `[":age", "between", [18, 65]]` -- which is
told apart from a nested condition by its shape: a condition's second element names an operator.
...
* **IN (`in`)** -- multiple point lookups on the index; with an ascending `orderBy()` on the same property the rows come back in key order
* **BETWEEN (`between`)** -- range scan with inclusive bounds
* **AND logic** -- uses the best available index (prefers unique indexes)
* **OR logic** -- uses both indexes when both sides are indexed, even when they are on properties of different types; falls back to full scan when either side is not indexed
* **Composite indexes** -- an AND chain of equality conditions covering a leading prefix of a composite (multi-property) index's properties uses that index too, even when not every property is bound; if the query's `orderBy()` matches the trailing indexed property, the scan also satisfies the ordering without a separate in-memory sort
