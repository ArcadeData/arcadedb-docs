...
All the changes executed via xref:reference/sql/sql-alter-database.adoc#sql-alter-database[SQL Alter Database] command are relative to the current database only and are persistent.
...
|`externalPropertyBucketDefaultPageSize`|Default page size in bytes for paired external-property buckets. They hold heavy property payloads (vector embeddings, large strings, embedded JSON) so the default is larger than for primary buckets to reduce multi-page chunking. Matches the LSM-index default (262144).|Integer|262144
|`externalPropertyBucketPath`|Filesystem directory where new paired external-property buckets are created. If empty (default), external buckets sit alongside primary buckets in the database directory. Set to a path on cheaper or slower storage (HDD, network mount) to tier the heavy payloads away from the topology files. The directory must exist and be writable. Existing external buckets are not relocated when this changes.|String|(empty)
