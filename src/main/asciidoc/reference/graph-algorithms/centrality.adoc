...
| config map | Map | No | see defaults | Configuration properties (all optional)
| `normalized` | Boolean | No | `true` | Divide scores by `1/((n-1)(n-2))` to normalize to [0,1]
|===
...
Measures how often a vertex lies on the shortest path between all other pairs, using the Brandes algorithm with stack-based back-propagation. Traversal follows outgoing edges only (directed). When `normalized` is `true`, scores are divided by `1/((n-1)(n-2))` (the directed normalization factor, matching NetworkX and Neo4j GDS).
