...
Custom functions defined via SQL's `DEFINE FUNCTION` statement can be called using the `CALL` clause with their namespace.

[source,sql]
----
// First, define a custom function in SQL
DEFINE FUNCTION math.add "SELECT :a + :b AS result" PARAMETERS [a,b] LANGUAGE sql
----

[source,cypher]
----
// Then call it from Cypher
CALL math.add(3, 5)
// Returns: {value: 8}

// Call with different parameter types
CALL my.greet('World')
