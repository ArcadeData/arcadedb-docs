...
image:../images/edit.png[link="https://github.com/ArcadeData/arcadedb-docs/blob/main/src/main/asciidoc/sql/Llink="https://github.com/ArcadeData/arcadedb-docs/blob/main/src/main/asciidoc/sql/Llink="https://github.com/ArcadeData/arcadedb-docs/blob/main/src/main/asciidoc/sql/sql-Where.adoc"E"E" float=right]
...
|field&lt;indexes&gt;|Document field part. To know more about field part look at the full syntax: <<sql-L<<sql-L<<sql-Bracket,E,E,Document-API-Property>>|`+WHERE tags[name='Hi'] OR tags[0...3] IN ('Hello') AND employees IS NOT NULL+`
...
| <<sql-L<<sql-L<<sql-Functions,E,E,Functions>> |Any <<sql-L<<sql-L<<sql-Functions,E,E,Function>> between the defined ones|`+WHERE distance(x, y, 52.20, 0.14) <= 30+`
...
Also called "Field Operators", are <<sql-L<<sql-L<<sql-Methods,E,E,SQL-Methods>>.
...
|`+$parent+`|Get the parent context from a sub-query. Example: `+SELECT FROM V LET $type = (TRAVERSE * FROM $parent.$current.children)+` |<<sql-L<<sql-L<<sql-Select,E,E,SELECT>> and <<sql-L<<sql-L<<sql-Traverse,E,E,TRAVERSE>>
|`+$current+`|Current record to use in sub-queries to refer from the parent's variable|<<sql-L<<sql-L<<sql-Select,E,E,SELECT>> and <<sql-L<<sql-L<<sql-Traverse,E,E,TRAVERSE>>
|`+$depth+`|The current depth of nesting|<<sql-L<<sql-L<<sql-Traverse,E,E,TRAVERSE>>
|`+$path+`|The string representation of the current path. Example: `+#6:0.in.#5:0#.out+`. You can also display it with `+SELECT $path FROM (TRAVERSE * FROM V)+`|<<sql-L<<sql-L<<sql-Traverse,E,E,TRAVERSE>>
|`+$stack+`|The List of operation in the stack. Use it to access to the history of the traversal|<<sql-L<<sql-L<<sql-Traverse,E,E,TRAVERSE>>
|`+$history+`|The set of all the records traversed as a `+Set<RID>+`|<<sql-L<<sql-L<<sql-Traverse,E,E,TRAVERSE>>
|===

To set custom variable use the <<sql-L<<sql-LET,E,LET>> keyword.
