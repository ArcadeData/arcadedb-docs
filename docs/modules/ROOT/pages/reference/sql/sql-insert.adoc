...
[discrete]
= SQL - `INSERT`
image:edit.png[link="https://github.com/ArcadeData/arcadedb-docs/blob/main/src/main/asciidoc/reference/sql/sql-insert.adoc" float=right]
...
NOTE: To insert only documents, vertices, or edges with a certain *unique* property, use a xref:reference/sql/sql-create-index.adoc#sql-create-index[unique index].
...
* Inserts using xref:reference/sql/sql-select.adoc#sql-select[`SELECT`] sub-queries
...
Besides the specialized command xref:reference/sql/sql-create-vertex.adoc#sql-create-vertex[CREATE VERTEX],
...
However, edges have to be created with xref:reference/sql/sql-create-edge.adoc#sql-create-edge[CREATE EDGE].
