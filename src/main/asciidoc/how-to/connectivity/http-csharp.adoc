...
[TIP]
====
*Connection options for C# / .NET*

ArcadeDB exposes multiple wire protocols, so you can pick the one your stack already speaks:

* xref:how-to/connectivity/bolt.adoc[*Neo4j BOLT*] -- `Install-Package Neo4j.Driver`, run Cypher queries.
* xref:how-to/connectivity/postgres.adoc[*PostgreSQL wire protocol*] -- `Install-Package Npgsql`, run SQL.
* xref:reference/http-api/http.adoc[*HTTP/JSON API*] -- use `HttpClient`, no driver required.
