...
image:edit.png[link="https://github.com/ArcadeData/arcadedb-docs/blob/main/src/main/asciidoc/reference/graphql/graphql.adoc" float=right]
...
If you're using ArcadeDB as xref:tutorials/embed-server.adoc#embedded-server[embedded], please add the dependency to the `arcadedb-graphql` library.
...
- HTTP API by using xref:reference/http-api/http.adoc#http-executecommand[`/command`] and xref:reference/http-api/http.adoc#http-executequery[`/query`] commands and `"graphql"` as language
- xref:how-to/connectivity/postgres.adoc#postgres-driver[Postgres Driver] by prefixing with `{graphql}` your query to execute.
...
If you're using the xref:tutorials/multimodel.adoc#document-model[Document Model] and links to connect documents, then you can map 1-1 the GraphQL type to ArcadeDB type.
Example:
...
If you're using a xref:tutorials/multimodel.adoc#graph-model[Graph Model] for your domain, then you need to declare with a GraphQL directive how the relationship is translated on the graph model.
...
Example by using Java API (but the same by using xref:reference/http-api/http.adoc#http-executecommand[HTTP Command] API):
...
Executes a xref:reference/sql/chapter.adoc#sql[SQL] query.
The query can use parameters passed at invocation time.

Example of definition of a query using xref:reference/sql/chapter.adoc#sql[SQL] in GraphQL:
...
Executes a xref:reference/gremlin/gremlin.adoc#gremlin-api[Gremlin] query.
The query can use parameters passed at invocation time.

Example of definition of a query using xref:reference/gremlin/gremlin.adoc#gremlin-api[Gremlin] in GraphQL:
...
Executes a xref:reference/cypher/chapter.adoc#cypher[Cypher] query.
The query can use parameters passed at invocation time.

Example of definition of a query using xref:reference/cypher/chapter.adoc#cypher[Cypher] in GraphQL:
...
Mark the field as the record identity or xref:concepts/basics.adoc#rid[].
