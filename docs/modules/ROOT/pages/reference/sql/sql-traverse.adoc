...
[discrete]
= SQL - `TRAVERSE`
image:edit.png[link="https://github.com/ArcadeData/arcadedb-docs/blob/main/src/main/asciidoc/reference/sql/sql-traverse.adoc" float=right]
...
NOTE: In many cases, you may find it more efficient to use xref:reference/sql/sql-select.adoc#sql-select[`SELECT`], which can result in shorter and faster queries. For more information, see <<traverse-versus-select,`TRAVERSE` versus `SELECT`>> below.
...
NOTE: The use of the xref:reference/sql/sql-where.adoc#filtering[`WHERE`] clause has been deprecated for this command.
...
NOTE: You can also define the maximum depth in the xref:reference/sql/sql-select.adoc#sql-select[`SELECT`] command, but it's much more efficient to set it at the inner <<sql-traverse,`TRAVERSE`>> statement because the returning record sets are already filtered by depth.

* Combine traversal with xref:reference/sql/sql-select.adoc#sql-select[`SELECT`] command to filter the result-set. Repeat the above example, filtering for users in Rome:
[source,sql]
----
ArcadeDB> SELECT FROM (TRAVERSE out("Friend") FROM #10:1234 MAXDEPTH 3)
            WHERE city = 'Rome'
...
In addition to his, you can specify the fields at a type-level. xref:concepts/inheritance.adoc#inheritance[Inheritance] is supported. By specifying `Person.city` and the type `Customer` extends person, you also traverse fields in `Customer`.
...
* *`$path`* Gives a string representation of the current path. For instance, `#5:0.out`. You can also display it through xref:reference/sql/sql-select.adoc#sql-select[`SELECT`]:
[source,sql]
----
...
When you already know traversal information, such as relationship names and depth-level, consider using xref:reference/sql/sql-select.adoc#sql-select[`SELECT`] instead of <<sql-traverse,`TRAVERSE`>> as it is faster in some cases.
...
But, you could also express this same query using xref:reference/sql/sql-select.adoc#sql-select[`SELECT`] operation, in a way that is also shorter and faster:
...
While you can use the <<sql-traverse,`TRAVERSE`>> command with any domain model, it provides the greatest utility with the xref:tutorials/multimodel.adoc#graph-model[Graph Model].
