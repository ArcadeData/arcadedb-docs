...
* **xref:concepts/vector-search.adoc#vector-model[Vector Similarity]** -- Product and document embeddings with LSM_VECTOR, HNSW, and DiskANN indexes
* **xref:tutorials/multimodel.adoc#searchengine-model[Full-Text Search]** -- Hybrid keyword + semantic search with reciprocal rank fusion
* **xref:tutorials/multimodel.adoc#document-model[Document Model]** -- Faceted filtering on product attributes
