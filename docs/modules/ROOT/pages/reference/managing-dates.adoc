[[managing-dates]]
= Managing Dates
image:edit.png[link="https://github.com/ArcadeData/arcadedb-docs/blob/main/src/main/asciidoc/reference/managing-dates.adoc" float=right]
...
In the event that these default formats are not sufficient for the needs of your application, you can customize them through xref:reference/sql/sql-alter-database.adoc#sql-alter-database[`ALTER DATABASE ... DATEFORMAT`] and xref:reference/sql/sql-alter-database.adoc#sql-alter-database[`DATETIMEFORMAT`] commands.
For instance,
...
|===
| SQL | Description
| xref:reference/sql/sql-functions.adoc#date[`date()`] | Function converts dates to and from strings and dates, also uses custom formats.
| xref:reference/sql/sql-functions.adoc#sysdate[`sysdate()`] | Function returns the current date.
| [[format]]<<format,`.format()`>> | Method returns the date in different formats.
| xref:reference/sql/sql-methods.adoc#asdate[`.asDate()`] | Method converts any type into a date.
| xref:reference/sql/sql-methods.adoc#asdatetime[`.asDatetime()`] | Method converts any type into datetime.
| xref:reference/sql/sql-methods.adoc#aslong[`.asLong()`] | Method converts any date into long format, (that is, Unix time).
| xref:reference/sql/sql-methods.adoc#precision[`.precision()`] | Method modifies the precision of a datetime property. For example, `.precision('millisecond')` returns the datetime property with millisecond precision
|===

For example, consider a case where you need to extract only the years for date entries and to arrange them in order.  You can use the xref:reference/sql/sql-methods.adoc#format-method[`.format`] method to extract dates into different formats.
...
Here, you again create a vertex for the traditional date of the foundation of Rome.  However, instead of altering the database, you format the date field in xref:reference/sql/sql-create-vertex.adoc#sql-create-vertex[`CREATE VERTEX`] command.

[discrete]
#### Viewing Unix Time

In addition to the formatted date and datetime, you can also view the underlying count from the Unix Epoch, using the xref:reference/sql/sql-methods.adoc#aslong[`asLong()`] method for records.  For example,
...
The units of time used in the xref:reference/sql/sql-functions.adoc#duration[`duration()`] function and xref:reference/sql/sql-methods.adoc#precision[`precision()`] method
