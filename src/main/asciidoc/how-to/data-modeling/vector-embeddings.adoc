...
* **The auto-sized caches budget against available heap, not total heap.** `graphBuildCacheMaxHeapPercent` and `searchCacheMaxHeapPercent` are shares of the heap actually free at the time, so a rebuild holding the old graph resident asks for a smaller cache instead of the same one. Previously they were shares of `-Xmx`, which meant a larger heap grew the caches proportionally and a rebuild that did not fit still did not fit after raising it.
