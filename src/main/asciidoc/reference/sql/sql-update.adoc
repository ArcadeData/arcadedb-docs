...
+
NOTE: Because `BATCH` commits as it goes, an `UPDATE ... BATCH` is not atomic: a failure part way through
leaves the records committed up to the last batch boundary in place. For the same reason, running it inside
a transaction opened by your application makes that transaction no longer atomic either, and a rollback
cannot undo what the batches already committed. A `transaction()` block with retries does not retry in that
case: the conflict is propagated to your application instead, since repeating the block would apply the
already committed records a second time. Since 26.10.1 a SQL Script
<<sql-script-commit,`BEGIN ... COMMIT RETRY <n>`>> block behaves the same way.
...
* Update a single map key or list position in place

----
ArcadeDB> UPDATE Account SET settings.theme = 'dark'
ArcadeDB> UPDATE Account SET settings['theme'] = 'dark'
ArcadeDB> UPDATE Account SET addresses[0] = 'Foo'
----

NOTE: Before 26.10.1 these nested assignments were reported as successful but were never stored. If you relied on them
on an earlier release, re-check the affected records. Before the same release, a positional assignment on a property
whose value is a *set* always failed with a `ConcurrentModificationException` and left the value truncated.

NOTE: A positional assignment past the end of a collection, such as `addresses[5] = 'Foo'` on a 3-element list, pads
the missing positions with `null`. A property whose value is a *set* is the exception: a set cannot hold `null` more
than once, so several missing positions collapse into one and the value lands right after it -
`[a, b, c, null, Foo]` rather than `[a, b, c, null, null, Foo]`. Declare the property as `LIST` if every position
must be addressable.
...
The newly created record is a real vertex, so it can immediately participate in graph traversals and be the source/target of <<sql-create-edge,`CREATE EDGE`>>.
...
The `UPSERT` clause only guarantees atomicity when you use a `UNIQUE` index and perform the look-up on the index through the <<filtering,`WHERE`>> condition. This applies to both document and vertex types.
...
* `UPSERT` is not available on edge types. To create an edge only if it does not already exist, use <<sql-create-edge,`CREATE EDGE ... IF NOT EXISTS`>>.
