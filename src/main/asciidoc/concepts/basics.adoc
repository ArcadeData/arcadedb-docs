...
Each bucker is one file at file system level.
...
The combination of types and buckets is very powerful and has a number of use cases. You should also use Types in your applications, but you can address a specific bucket based on your use case and how you decide to split the records between buckets.
...
By using the explicit cluster instead of the logical type, this query runs significantly faster, because ArcadeDB can narrow the search to the targeted bucket.
