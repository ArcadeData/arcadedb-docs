...
TIP: INT8 quantization is recommended for production use. It stores vectors in compact index pages instead of full documents, providing 2.5x faster search and 4x lower memory usage with negligible accuracy loss. See <<quantization-performance>> for benchmarks.
...
- `maxConnections`: Graph connections per node, per layer -- the Vamana degree (default: 32). Not doubled at the base layer like hnswlib's `M`, so set `maxConnections = 2 * M` to match an hnswlib config. Increase to 48-64 for better recall in large datasets
...
- *Default*: `0` (auto-sized: the whole corpus, as long as it fits `arcadedb.vectorIndex.graphBuildCacheMaxHeapPercent` of the `-Xmx` ceiling)
- *Per-index metadata*: `"graphBuildCacheSize": 5000`
- *Global setting*: `arcadedb.vectorIndex.graphBuildCacheSize`
- *Recommended*: leave at `0` and let the heap percentage bound it. Pin an explicit value only to cap the build's footprint on a heap shared with other work; a cache smaller than the corpus makes the build re-read vectors it has already seen, which is by far the largest term in build time
...
NOTE: The inline vectors are stored as full-precision `float32` whatever the index quantization is - quantization applies to the index data, not to this copy. Size the `.vecgraph` file accordingly: roughly `vectors x dimensions x 4 bytes`, plus the graph topology. A 1M x 768 index is therefore about 3 GB on disk even with `INT8` quantization enabled.
...
. *Start with INT8*: For most use cases, INT8 provides the best balance. Only use NONE if accuracy loss is unacceptable.

. *Measure Recall on Your Data*: Quantization accuracy depends on your embedding model and data distribution. Always benchmark on representative queries:
...
. *Combine with Graph Storage*: For large indexes (100K+ vectors), combine quantization with `storeVectorsInGraph=true`:
...
. *Monitor Fetch Sources*: Use metrics to verify quantization is working:
...
. *Quantization is Transparent*: Vectors are automatically quantized on insert and dequantized on retrieval. No query changes needed:
...
. *Two-Stage Search with BINARY*: For very large datasets, use BINARY quantization as a fast pre-filter, then rerank with full precision:
...
Use the `vector.neighbors()` function to perform similarity searches:
...
NOTE: After `expand()`, each row contains the document's properties and a `distance` field. You can filter with a WHERE clause in the outer query (e.g., `SELECT ... FROM (SELECT expand(...)) WHERE source = 'wiki'`) or in your application code.
...
SELECT expand(vector.neighbors('MultiModalRecord[imageEmbedding]', [0.1, 0.2, ...], 5))

-- Query by text modality
SELECT expand(vector.neighbors('MultiModalRecord[textEmbedding]', [0.2, 0.3, ...], 5))
----
...
*Note*: The first argument to `vector.neighbors()` must always be the *index name* in `'TypeName[propertyName]'` format (e.g., `'Document[embedding]'`). Without `expand()`, you get a single row with the `neighbors` field containing the entire list. Use `expand()` to flatten the results into individual rows where you can access document properties and `distance` directly.
...
[source,sql]
----
SELECT expand(vector.neighbors('ImageEmbedding[vector]', $queryVector, 50))
----
...
[source,sql]
----
SELECT expand(vector.neighbors('Embedding[vector]', $queryVector, 50))
----
...
For pre-quantized ingest from an int8 embedding provider, declare the property as `BINARY` and call `withEncoding(VectorEncoding.INT8)` (or the string overload). The factory rejects mismatches between property type and encoding at create time:

[source,java]
----
import com.arcadedb.index.vector.VectorEncoding;

// Property MUST be BINARY when encoding=INT8
schema.createProperty("Document", "embedding", Type.BINARY);
...
final LSMVectorIndexBuilder builder = new LSMVectorIndexBuilder(
    ...
    .withDimensions(1024)
    .withSimilarity(VectorSimilarityFunction.COSINE)
    .withEncoding(VectorEncoding.INT8);  // wire/storage encoding, not internal quantization

final LSMVectorIndex index = builder.create();
----

See <<vector-int8-ingest,INT8 Pre-Quantized Ingest>> for the trade-offs and when to choose this over `quantization=INT8`.
...
final JSONObject metadata = new JSONObject()
    ...
    .put("encoding", "FLOAT32")         // Optional: FLOAT32 (default) or INT8 for pre-quantized ingest
...
3. *Tune Performance Parameters*: Start with defaults (maxConnections=32, beamWidth=100) and increase for better recall if needed.
...
SELECT expand(vector.neighbors('Document[embedding]', $queryEmbedding, 10))
----
...
SELECT expand(vector.neighbors('Image[features]', $imageFeatures, 5))
----
...
SELECT expand(vector.neighbors('Product[embedding]', $userPreferenceVector, 20))
----
...
* <<vector-neighbors-ext, vector.neighbors()>>
