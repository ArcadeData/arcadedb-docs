...
This is a common practice to pass secrets to containers, for example with Docker compose, and it is how
the https://github.com/ArcadeData/arcadedb/blob/main/package/src/main/config/arcadedb-statefulset.yaml[example Kubernetes manifest] supplies the root password, from a mounted `Secret`.
A trailing newline, which most editors and `echo` append, is not treated as part of the password, and an
empty file is rejected with an error naming it rather than silently accepted (_since v26.9.1_).

The same file-based approach is available for the High Availability cluster token using:
```
-Darcadedb.ha.clusterTokenPath=/path/to/cluster_token
```
It is read only when `arcadedb.ha.clusterToken` is not set, and the file content is trimmed of surrounding whitespace,
so a trailing newline added by most editors or `echo` does not break the token comparison between nodes (_since v26.6.1_).
